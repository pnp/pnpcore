...
namespace Demo.ConsoleApp
{
    class Program
    {
        public static async Task Main(string[] args)
        {
            ...
            var customSettingsSection = builder.Configuration.GetSection("CustomSettings");
            customSettingsSection.Bind(customSettings);
            ...
            {
                options.Credentials.Configurations.Add("interactive", new PnP.Core.Auth.Services.Builder.Configuration.PnPCoreAuthenticationCredentialConfigurationOptions
                    {
                ...
                options.Credentials.Configurations.Add("credentials", new PnP.Core.Auth.Services.Builder.Configuration.PnPCoreAuthenticationCredentialConfigurationOptions
                    {
                        ClientId = customSettings.ClientId,
                        TenantId = customSettings.TenantId,
                        Interactive = new PnP.Core.Auth.Services.Builder.Configuration.PnPCoreAuthenticationInteractiveOptions
                ...
                options.Sites.Add("DemoSite", new PnP.Core.Auth.Services.Builder.Configuration.PnPCoreAuthenticationSiteOptions
                    {
            ...
            using (var scope = host.Services.CreateScope())
            {
                ...
                await RunScoped(pnpContextFactory);
            }

            host.Dispose();
        }

        private static async Task RunScoped(IPnPContextFactory pnpContextFactory)
        {
            ...
            using (var context = pnpContextFactory.Create("DemoSite"))
            {
                ...
                {
                    var take2Messages = System.Linq.Queryable.Take(team.PrimaryChannel.Messages, 2);

                    Console.ForegroundColor = ConsoleColor.Yellow;
                    Console.WriteLine("===Next page loaded==");
                    Console.WriteLine($"Current number of messages: {take2Messages.AsEnumerable().Count()}");
                    Console.ResetColor();

                    Console.ForegroundColor = ConsoleColor.Yellow;
                    Console.WriteLine("===Next page loaded==");
                    Console.WriteLine($"Current number of messages: {take2Messages.Skip(2).AsEnumerable().Count()}");
                    Console.ResetColor();
            ...
            var pageName = $"Page-{Guid.NewGuid():N}.aspx";
