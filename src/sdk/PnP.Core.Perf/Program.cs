...
namespace PnP.Core.Perf
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            ResponseParsingBenchMark rpb = new ResponseParsingBenchMark();
            var current = rpb.Current();
            var tuned = rpb.Tuned();

            Console.ForegroundColor = ConsoleColor.Red;
            Console.WriteLine($"Are equal: {current == tuned}");
            Console.ResetColor();
            ...
            var summary = BenchmarkRunner.Run<ResponseParsingBenchMark>();
#endif
        }
    }
}
