...
namespace PnP.Core.Test.Teams
{
    ...
    public class TeamChatMessageTests
    {
        ...
        [TestMethod]
        public async Task GetChatMessageRepliesAsyncTest()
        {
            //TestCommon.Instance.Mocking = false;
            using (var context = await TestCommon.Instance.GetContextAsync(TestCommon.TestSite))
            {
                ...
                // assume as if there are no chat messages
                // There appears to be no remove option yet in this feature - so add a recognisable message
                var body = "Hello, this is a unit test (GetChatMessageRepliesAsyncTest) posting a message - PnP Rocks!";
                await chatMessages.AddAsync(body);
                channel = await channel.GetAsync(o => o.Messages);
                var updateMessages = channel.Messages.AsRequested();
                ...
                var replyBody = "Hello, this is a unit test (GetChatMessageRepliesAsyncTest) posting a reply message - PnP Rocks!";
                var reply = await message.AddReplyAsync(replyBody);

                Assert.IsNotNull(reply.CreatedDateTime);
                // Depending on regional settings this check might fail
                //Assert.AreEqual(message.DeletedDateTime, DateTime.MinValue);
                Assert.IsNotNull(reply.Etag);
                Assert.IsNotNull(reply.Importance);
                Assert.IsNotNull(reply.LastModifiedDateTime);
                Assert.IsNotNull(reply.Locale);
                Assert.IsNotNull(reply.MessageType);
                Assert.IsNotNull(reply.WebUrl);
                Assert.AreEqual(reply.Body.Content, replyBody);
            }
        }
        ...
        [TestMethod]
        public void AddChatMessageReplyTest()
        {
            //TestCommon.Instance.Mocking = false;
            using (var context = TestCommon.Instance.GetContext(TestCommon.TestSite))
            {
                ...
                // assume as if there are no chat messages
                // There appears to be no remove option yet in this feature - so add a recognisable message
                var body = $"Hello, this is a unit test (AddChatMessageReplyTest) posting a message - PnP Rocks! - Woah...";
                var newMessage = chatMessages.Add(body);

                channel = channel.Get(o => o.Messages);
                var updateMessages = channel.Messages.AsRequested();

                var message = updateMessages.First(o => o.Id == newMessage.Id);

                var replyContent = "This is a reply to a Channel message";
                var reply = message.AddReply(replyContent);

                message.Load(y => y.Replies);
        ...
        [TestMethod]
        public async Task AddChatMessageReplyWithTagMentionAsyncTest()
        {
            //TestCommon.Instance.Mocking = false;
            using (var context = await TestCommon.Instance.GetContextAsync(TestCommon.TestSite))
            {
                ...
                var bodyReply = $"Hello, PnP Rocks!<br/>This is a Tag mention reply test<br/>Mention 1: <at id=\"0\">Tag 1</at>";
                ...
                var messageBody = $"Hello, this is a unit test (AddChatMessageReplyWithTagMentionAsyncTest) posting a message - PnP Rocks! - Woah...";


                var chatMessage = await chatMessages.AddAsync(messageBody);
                ...
                await message.AddReplyAsync(messageOptions);
                ...
                Assert.AreEqual(message.Replies.Length, 1);
                Assert.AreEqual(firstReply.Body.Content, firstReply.Body.Content);
        ...
        [TestMethod]
        public async Task AddChatMessageReplyWithUserAndChannelMentionAsyncTest()
        {
            //TestCommon.Instance.Mocking = false;
            using (var context = await TestCommon.Instance.GetContextAsync(TestCommon.TestSite))
            {
                ...
                var bodyReply = $"Hello, PnP Rocks! <br/>This is a channel and user mention test <br/>Mention 1: <at id=\"0\">Channel</at><br/>Mention 2: <at id=\"1\">User</a>";
                ...
                Assert.AreEqual(message.Replies.Length, 1);
                Assert.AreEqual(firstReply.Body.Content, firstReply.Body.Content);
                Assert.IsTrue(firstReply.Mentions.Length == 2);
                Assert.IsTrue(firstReply.Mentions.FirstOrDefault().Mentioned.Conversation.Id == channel.Id);
                Assert.IsTrue(firstReply.Mentions.Last().Mentioned.User.Id == graphUser.Id);

            }
        }

        [TestMethod]
        public async Task AddChatMessageReplyWithAttachmentAsyncTest()
        {
            //TestCommon.Instance.Mocking = false;
            using (var context = await TestCommon.Instance.GetContextAsync(TestCommon.TestSite))
            {
                ...
                var body = $"<h1>Hello</h1><br />This is a unit test reply with a file attachment (AddChatMessageReplyWithAttachmentAsyncTest) posting a message - <attachment id=\"{attachmentId}\"></attachment>";
                ...
                var messageBody = $"Hello, this is a unit test (AddChatMessageReplyWithAttachmentAsyncTest) posting a message - PnP Rocks! - Woah...";
                ...
                Assert.AreEqual(message.Replies.Length, 1);
                Assert.AreEqual(firstReply.Body.Content, firstReply.Body.Content);
                Assert.IsTrue(firstReply.Attachments.Length == 1);
                Assert.IsTrue(firstReply.Attachments.FirstOrDefault().Name == existingFile.Name);

            }
        }

        [TestMethod]
        public async Task AddChatMessageReplyWithInlineImagesAsyncTest()
        {
            //TestCommon.Instance.Mocking = false;
            using (var context = await TestCommon.Instance.GetContextAsync(TestCommon.TestSite))
            {
                ...
                var body = $"<div><div><h1>Hello</h1><p>This is a unit test (AddChatMessageReplyWithInlineImagesAsyncTest) posting a message with inline image</p><div><span><img height=\"392\" src=\"../hostedContents/1/$value\" width=\"300\" style=\"vertical-align:bottom; width:300px; height:392px\"></span></div></div></div>";
