...
namespace PnP.Core.Test.Base
{
    ...
    public class DataModelExtensionTests
    {
        ...
        public async Task EnsurePropertiesCollectionWithIncludePartiallyLoadedRecursive()
        {
            ...
            using (var context = await TestCommon.Instance.GetContextAsync(TestCommon.TestSite))
            {
                await context.Web.GetAsync(p => p.WelcomePage, p => p.Lists.Query(p => p.Title, p => p.Description, p => p.Fields.QueryProperties(p => p.Title)));
                ...
                await context.Web.EnsurePropertiesAsync(p => p.Lists.Query(p => p.Title, p => p.Description, p => p.Fields.QueryProperties(p => p.StaticName)), p => p.WelcomePage);
                var web = context.Web;
            ...
            {
                await context.Web.GetAsync(p => p.Lists.Query(p => p.Title, p => p.Description, p => p.Fields.QueryProperties(p => p.StaticName)), p => p.WelcomePage);
