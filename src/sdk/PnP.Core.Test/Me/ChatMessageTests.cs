using Microsoft.VisualStudio.TestTools.UnitTesting;
using PnP.Core.Model;
using PnP.Core.Model.Me;
using PnP.Core.Model.SharePoint;
using PnP.Core.Model.Teams;
using PnP.Core.QueryModel;
using PnP.Core.Test.Utilities;
using System;
using System.IO;
using System.Linq;
using System.Threading.Tasks;

namespace PnP.Core.Test.Me
{
    [TestClass]
    public class ChatMessageTest
    {
        private readonly string GroupChatTopic = "Test PnP Group Chat!";

        [ClassInitialize]
        public static void TestFixtureSetup(TestContext context)
        {
            // Configure mocking default for all tests in this class, unless override by a specific test
            // TestCommon.Instance.Mocking = false;
        }

        [TestMethod]
        public async Task GetChatMessageOneOnOneAsyncTest()
        {
            //TestCommon.Instance.Mocking = false;
            using (var context = await TestCommon.Instance.GetContextAsync(TestCommon.TestSite))
            {
                var me = await context.Me.GetAsync(o => o.Chats);
                Assert.IsTrue(me.Chats.Length > 0);

                var chat = me.Chats.AsRequested().FirstOrDefault(i => i.ChatType == ChatTypeConstants.OneOnOne);
                Assert.IsNotNull(chat);

                chat = await chat.GetAsync(o => o.Messages);
                var chatMessages = chat.Messages;

                Assert.IsNotNull(chatMessages);

                // assume as if there are no chat messages
                // There appears to be no remove option yet in this feature - so add a recognisable message
                var body = "Hello, this is a unit test (GetChatMessageOneOnOneAsyncTest) posting a message - PnP Rocks!";
                await chatMessages.AddAsync(body);

                chat = await chat.GetAsync(o => o.Messages);
                var updateMessages = chat.Messages.AsRequested();

                var message = updateMessages.First(o => o.Body.Content == body);
                Assert.IsNotNull(message.CreatedDateTime);
                Assert.IsNotNull(message.Etag);
                Assert.IsNotNull(message.Importance);
                Assert.IsNotNull(message.LastModifiedDateTime);
                Assert.IsNotNull(message.Locale);
                Assert.IsNotNull(message.MessageType);

                Assert.IsTrue(message.IsPropertyAvailable(o => o.ReplyToId));
                Assert.IsNull(message.ReplyToId);
                Assert.IsTrue(message.IsPropertyAvailable(o => o.Subject));
                Assert.IsNull(message.Subject);
                Assert.IsTrue(message.IsPropertyAvailable(o => o.Summary));
                Assert.IsNull(message.Summary);
            }
        }


        [TestMethod]
        public void AddChatMessageOneOnOneChatTest()
        {
            //TestCommon.Instance.Mocking = false;
            using (var context = TestCommon.Instance.GetContext(TestCommon.TestSite))
            {
                var me = context.Me.Get(o => o.Chats);
                Assert.IsTrue(me.Chats.Length > 0);

                var chat = me.Chats.AsRequested().FirstOrDefault(i => i.ChatType == ChatTypeConstants.OneOnOne);
                Assert.IsNotNull(chat);

                chat.Load(o => o.Messages);
                var chatMessages = chat.Messages;

                Assert.IsNotNull(chatMessages);

                // assume as if there are no chat messages
                // There appears to be no remove option yet in this feature - so add a recognisable message
                var body = $"Hello, this is a unit test (AddChatMessageToOneOnOneChatTest) posting a message - PnP Rocks! - Woah...";
                chatMessages.Add(body);

                chat = chat.Get(o => o.Messages);
                var updateMessages = chat.Messages.AsRequested();

                var message = updateMessages.First(o => o.Body.Content == body);
                Assert.IsNotNull(message.CreatedDateTime);
                Assert.IsNotNull(message.Etag);
                Assert.IsNotNull(message.Importance);
                Assert.IsNotNull(message.LastModifiedDateTime);
                Assert.IsNotNull(message.Locale);
                Assert.IsNotNull(message.MessageType);

                Assert.IsTrue(message.IsPropertyAvailable(o => o.ReplyToId));
                Assert.IsNull(message.ReplyToId);
                Assert.IsTrue(message.IsPropertyAvailable(o => o.Subject));
                Assert.IsNull(message.Subject);
                Assert.IsTrue(message.IsPropertyAvailable(o => o.Summary));
                Assert.IsNull(message.Summary);
            }
        }

        [TestMethod]
        public void AddChatMessageOptionsOneOnOneTest()
        {
            //TestCommon.Instance.Mocking = false;
            using (var context = TestCommon.Instance.GetContext(TestCommon.TestSite))
            {
                var me = context.Me.Get(o => o.Chats);
                Assert.IsTrue(me.Chats.Length > 0);

                var chat = me.Chats.AsRequested().FirstOrDefault(i => i.ChatType == ChatTypeConstants.OneOnOne);
                Assert.IsNotNull(chat);

                chat.Load(o => o.Messages);
                var chatMessages = chat.Messages;

                Assert.IsNotNull(chatMessages);

                // assume as if there are no chat messages
                // There appears to be no remove option yet in this feature - so add a recognisable message
                var body = $"Hello, this is a unit test (AddChatMessageOptionsOneOnOneTest) posting a message - PnP Rocks! - Woah...";
                var subject = "Options";
                chatMessages.Add(new ChatMessageOptions()
                {
                    Content = body,
                    Subject = subject
                });

                chat = chat.Get(o => o.Messages);
                var updateMessages = chat.Messages.AsRequested();

                var message = updateMessages.First(o => o.Body.Content == body);
                Assert.IsNotNull(message.CreatedDateTime);
                Assert.IsNotNull(message.Etag);
                Assert.IsNotNull(message.Importance);
                Assert.IsNotNull(message.LastModifiedDateTime);
                Assert.IsNotNull(message.Locale);
                Assert.IsNotNull(message.MessageType);

                Assert.IsTrue(message.IsPropertyAvailable(o => o.ReplyToId));
                Assert.IsNull(message.ReplyToId);
                Assert.IsTrue(message.IsPropertyAvailable(o => o.Subject));
                Assert.IsNotNull(message.Subject);
                Assert.IsTrue(message.IsPropertyAvailable(o => o.Summary));
                Assert.IsNull(message.Summary);
            }
        }

        [TestMethod]
        public async Task AddChatMessageHtmlOneOnOneAsyncTest()
        {
            //TestCommon.Instance.Mocking = false;
            using (var context = await TestCommon.Instance.GetContextAsync(TestCommon.TestSite))
            {
                var me = context.Me.Get(o => o.Chats);
                Assert.IsTrue(me.Chats.Length > 0);

                var chat = me.Chats.AsRequested().FirstOrDefault(i => i.ChatType == ChatTypeConstants.OneOnOne);
                Assert.IsNotNull(chat);

                chat.Load(o => o.Messages);
                var chatMessages = chat.Messages;

                Assert.IsNotNull(chatMessages);

                // assume as if there are no chat messages
                // There appears to be no remove option yet in this feature - so add a recognisable message
                var body = $"<h1>Hello</h1><br />This is a unit test (AddChatMessageHtmlOneOnOneAsyncTest) posting a message - <strong>PnP Rocks!</strong> - Woah...";

                await chatMessages.AddAsync(body, ChatMessageContentType.Html);

                chat = await chat.GetAsync(o => o.Messages);
                var updateMessages = chat.Messages;

                var message = updateMessages.AsEnumerable().Last();
                Assert.IsNotNull(message.CreatedDateTime);
                // Depending on regional settings this check might fail
                //Assert.AreEqual(message.DeletedDateTime, DateTime.MinValue);
                Assert.IsNotNull(message.Etag);
                Assert.IsNotNull(message.Importance);
                Assert.IsNotNull(message.LastModifiedDateTime);
                Assert.IsNotNull(message.Locale);
                Assert.IsNotNull(message.MessageType);

                Assert.IsTrue(message.IsPropertyAvailable(o => o.ReplyToId));
                Assert.IsNull(message.ReplyToId);
                Assert.IsTrue(message.IsPropertyAvailable(o => o.Subject));
                Assert.IsNull(message.Subject);
                Assert.IsTrue(message.IsPropertyAvailable(o => o.Summary));
                Assert.IsNull(message.Summary);

            }
        }

        [TestMethod]
        public async Task AddChatMessageFileAttachmentOneOnOneAsyncTest()
        {
            //TestCommon.Instance.Mocking = false;
            using (var context = await TestCommon.Instance.GetContextAsync(TestCommon.TestSite))
            {
                var me = context.Me.Get(o => o.Chats);
                Assert.IsTrue(me.Chats.Length > 0);

                var chat = me.Chats.AsRequested().FirstOrDefault(i => i.ChatType == ChatTypeConstants.OneOnOne);
                Assert.IsNotNull(chat);

                chat.Load(o => o.Messages);
                var chatMessages = chat.Messages;

                Assert.IsNotNull(chatMessages);

                // Upload File to SharePoint Library - it will have to remain i guess as onetime upload.
                IFolder folder = await context.Web.Lists.GetByTitle("Documents").RootFolder.GetAsync();
                IFile existingFile = await folder.Files.FirstOrDefaultAsync(o => o.Name == "test_added.docx");
                if (existingFile == default)
                {
                    existingFile = await folder.Files.AddAsync("test_added.docx", System.IO.File.OpenRead($".{Path.DirectorySeparatorChar}TestAssets{Path.DirectorySeparatorChar}test.docx"));
                }

                Assert.IsNotNull(existingFile);
                Assert.AreEqual("test_added.docx", existingFile.Name);

                // Useful reference - https://docs.microsoft.com/en-us/graph/api/chatmessage-post?view=graph-rest-beta&tabs=http#example-4-file-attachments
                // assume as if there are no chat messages
                var attachmentId = existingFile.ETag.AsGraphEtag(); // Needs to be the documents eTag - just the GUID part
                var body = $"<h1>Hello</h1><br />This is a unit test with a file attachment (AddChatMessageFileAttachmentOneOnOneAsyncTest) posting a message - <attachment id=\"{attachmentId}\"></attachment>";

                var fileUri = new Uri(existingFile.LinkingUrl);

                await chatMessages.AddAsync(new ChatMessageOptions
                {
                    Content = body,
                    ContentType = ChatMessageContentType.Html,
                    Attachments = {
                        new ChatMessageAttachmentOptions
                        {
                            Id = attachmentId,
                            ContentType = "reference",
                            // Cannot have the extension with a query graph doesnt recognise and think its part of file extension - include in docs.
                            ContentUrl = new Uri(fileUri.ToString().Replace(fileUri.Query, "")),
                            Name = $"{existingFile.Name}",
                            ThumbnailUrl = null,
                            Content = null
                        }
                    }
                });

                chat = await chat.GetAsync(o => o.Messages);
                var updateMessages = chat.Messages;

                var message = updateMessages.AsEnumerable().Last();
                Assert.IsNotNull(message.CreatedDateTime);
                Assert.IsNotNull(message.Etag);
                Assert.IsNotNull(message.Importance);
                Assert.IsNotNull(message.LastModifiedDateTime);
                Assert.IsNotNull(message.Locale);
                Assert.IsNotNull(message.MessageType);

                Assert.IsTrue(message.IsPropertyAvailable(o => o.ReplyToId));
                Assert.IsNull(message.ReplyToId);
                Assert.IsTrue(message.IsPropertyAvailable(o => o.Subject));
                Assert.IsNull(message.Subject);
                Assert.IsTrue(message.IsPropertyAvailable(o => o.Summary));
                Assert.IsNull(message.Summary);

                //delete file
                await existingFile.DeleteAsync(); //Note this will break the link in the Teams chat.
            }
        }

        [TestMethod]
        public async Task AddChatMessageInlineImagesOneOnOneAsyncTest()
        {
            //TestCommon.Instance.Mocking = false;
            using (var context = await TestCommon.Instance.GetContextAsync(TestCommon.TestSite))
            {
                var me = context.Me.Get(o => o.Chats);
                Assert.IsTrue(me.Chats.Length > 0);

                var chat = me.Chats.AsRequested().FirstOrDefault(i => i.ChatType == ChatTypeConstants.OneOnOne);
                Assert.IsNotNull(chat);

                chat.Load(o => o.Messages);
                var chatMessages = chat.Messages;

                Assert.IsNotNull(chatMessages);

                // Useful reference - https://docs.microsoft.com/en-us/graph/api/chatmessage-post?view=graph-rest-beta&tabs=http#example-5-sending-inline-images-along-with-the-message
                // assume as if there are no chat messages

                var body = $"<div><div><h1>Hello</h1><p>This is a unit test (AddChatMessageInlineImagesOneOnOneAsyncTest) posting a message with inline image</p><div><span><img height=\"392\" src=\"../hostedContents/1/$value\" width=\"300\" style=\"vertical-align:bottom; width:300px; height:392px\"></span></div></div></div>";

                await chatMessages.AddAsync(new ChatMessageOptions
                {
                    Content = body,
                    ContentType = ChatMessageContentType.Html,
                    HostedContents =
                    {
                        new ChatMessageHostedContentOptions
                        {
                            Id = "1",
                            ContentBytes = "",
                            ContentType = "image/png"
                        }
                    }
                });

                chat = await chat.GetAsync(o => o.Messages);
                var updateMessages = chat.Messages;

                var message = updateMessages.AsEnumerable().Last();
                Assert.IsNotNull(message.CreatedDateTime);
                // Depending on regional settings this check might fail
                //Assert.AreEqual(message.DeletedDateTime, DateTime.MinValue);
                Assert.IsNotNull(message.Etag);
                Assert.IsNotNull(message.Importance);
                Assert.IsNotNull(message.LastModifiedDateTime);
                Assert.IsNotNull(message.Locale);
                Assert.IsNotNull(message.MessageType);
                Assert.IsNotNull(message.HostedContents);

                Assert.IsTrue(message.IsPropertyAvailable(o => o.ReplyToId));
                Assert.IsNull(message.ReplyToId);
                Assert.IsTrue(message.IsPropertyAvailable(o => o.Subject));
                Assert.IsNull(message.Subject);
                Assert.IsTrue(message.IsPropertyAvailable(o => o.Summary));
                Assert.IsNull(message.Summary);

            }
        }

        [TestMethod]
        public void AddChatMessageInlineImagesOneOnOneBatchTest()
        {
            //TestCommon.Instance.Mocking = false;
            using (var context = TestCommon.Instance.GetContext(TestCommon.TestSite))
            {
                var batch = context.NewBatch();

                var me = context.Me.GetBatch(batch, o => o.Chats);
                context.Execute(batch);

                var primaryChat = me.Result.Chats.AsRequested().FirstOrDefault(i => i.ChatType == ChatTypeConstants.OneOnOne);
                Assert.IsNotNull(primaryChat);

                batch = context.NewBatch();
                var chat = primaryChat.GetBatch(batch, o => o.Messages);
                context.Execute(batch);

                var chatMessages = chat.Result.Messages;
                Assert.IsNotNull(chatMessages);

                // Useful reference - https://docs.microsoft.com/en-us/graph/api/chatmessage-post?view=graph-rest-beta&tabs=http#example-5-sending-inline-images-along-with-the-message
                // assume as if there are no chat messages

                var body = $"<div><div><h1>Hello</h1><p>This is a unit test (AddChatMessageInlineImagesOneOnOneBatchTest) posting a message with inline image</p><div><span><img height=\"392\" src=\"../hostedContents/1/$value\" width=\"300\" style=\"vertical-align:bottom; width:300px; height:392px\"></span></div></div></div>";

                ITeamChatMessageHostedContentCollection coll = new TeamChatMessageHostedContentCollection
                {

                };

                batch = context.NewBatch();
                chatMessages.AddBatch(batch, new ChatMessageOptions
                {
                    Content = body,
                    ContentType = ChatMessageContentType.Html,
                    HostedContents = {
                            new ChatMessageHostedContentOptions
                            {
                                Id = "1",
                                ContentBytes = "",
                                ContentType = "image/png"
                            }
                        }
                });

                context.Execute(batch);

                var batch2 = context.NewBatch();

                chat = chat.Result.GetBatch(batch2, o => o.Messages);
                context.Execute(batch2);
                var updateMessages = chat.Result.Messages.AsRequested();

                var message = updateMessages.Last();
                Assert.IsNotNull(message.CreatedDateTime);
                Assert.IsNotNull(message.Etag);
                Assert.IsNotNull(message.Importance);
                Assert.IsNotNull(message.LastModifiedDateTime);
                Assert.IsNotNull(message.Locale);
                Assert.IsNotNull(message.MessageType);
                Assert.IsNotNull(message.HostedContents);

                Assert.IsTrue(message.IsPropertyAvailable(o => o.ReplyToId));
                Assert.IsNull(message.ReplyToId);
                Assert.IsTrue(message.IsPropertyAvailable(o => o.Subject));
                Assert.IsNull(message.Subject);
                Assert.IsTrue(message.IsPropertyAvailable(o => o.Summary));
                Assert.IsNull(message.Summary);
            }
        }

        [TestMethod]
        public async Task AddChatMessageFileAttachmentOptionsOneOnOneAsyncTest()
        {
            //TestCommon.Instance.Mocking = false;
            using (var context = await TestCommon.Instance.GetContextAsync(TestCommon.TestSite))
            {
                var me = context.Me.Get(o => o.Chats);
                Assert.IsTrue(me.Chats.Length > 0);

                var chat = me.Chats.AsRequested().FirstOrDefault(i => i.ChatType == ChatTypeConstants.OneOnOne);
                Assert.IsNotNull(chat);

                chat.Load(o => o.Messages);
                var chatMessages = chat.Messages;

                Assert.IsNotNull(chatMessages);

                // Upload File to SharePoint Library - it will have to remain i guess as onetime upload.
                IFolder folder = await context.Web.Lists.GetByTitle("Documents").RootFolder.GetAsync();
                IFile existingFile = await folder.Files.FirstOrDefaultAsync(o => o.Name == "test_added.docx");
                if (existingFile == default)
                {
                    existingFile = await folder.Files.AddAsync("test_added.docx", System.IO.File.OpenRead($".{Path.DirectorySeparatorChar}TestAssets{Path.DirectorySeparatorChar}test.docx"));
                }

                Assert.IsNotNull(existingFile);
                Assert.AreEqual("test_added.docx", existingFile.Name);

                // Useful reference - https://docs.microsoft.com/en-us/graph/api/chatmessage-post?view=graph-rest-beta&tabs=http#example-4-file-attachments
                // assume as if there are no chat messages
                var attachmentId = existingFile.ETag.AsGraphEtag(); // Needs to be the documents eTag - just the GUID part
                var body = $"<h1>Hello</h1><br />This is a unit test with a file attachment (AddChatMessageFileAttachmentOptionsOneOnOneAsyncTest) posting a message - <attachment id=\"{attachmentId}\"></attachment>";
                var subject = "Chat Message File Attachment Options Async Test";
                var fileUri = new Uri(existingFile.LinkingUrl);
                await context.ExecuteAsync();

                var batch = context.NewBatch();

                await chatMessages.AddBatchAsync(batch, new ChatMessageOptions
                {
                    Subject = subject,
                    Content = body,
                    ContentType = ChatMessageContentType.Html,
                    Attachments = {
                        new ChatMessageAttachmentOptions
                        {
                            Id = attachmentId,
                            ContentType = "reference",
                            // Cannot have the extension with a query graph doesnt recognise and think its part of file extension - include in docs.
                            ContentUrl = new Uri(fileUri.ToString().Replace(fileUri.Query, "")),
                            Name = $"{existingFile.Name}",
                            ThumbnailUrl = null,
                            Content = null
                        }
                    }
                });

                await context.ExecuteAsync(batch);

                chat = await chat.GetAsync(o => o.Messages);
                var updateMessages = chat.Messages;

                var message = updateMessages.AsEnumerable().OrderByDescending(o => o.Subject = subject).Last();
                Assert.IsNotNull(message.CreatedDateTime);
               
                Assert.IsNotNull(message.Etag);
                Assert.IsNotNull(message.Importance);
                Assert.IsNotNull(message.LastModifiedDateTime);
                Assert.IsNotNull(message.Locale);
                Assert.IsNotNull(message.MessageType);

                Assert.IsTrue(message.IsPropertyAvailable(o => o.ReplyToId));
                Assert.IsNull(message.ReplyToId);
                Assert.IsTrue(message.IsPropertyAvailable(o => o.Subject));
                Assert.IsNotNull(message.Subject);
                Assert.IsTrue(message.IsPropertyAvailable(o => o.Summary));
                Assert.IsNull(message.Summary);

                //delete file
                await existingFile.DeleteAsync(); //Note this will break the link in the Teams chat.

            }
        }

        [TestMethod]
        public async Task AddChatMessageSubjectOneOnOneAsyncTest()
        {
            //TestCommon.Instance.Mocking = false;
            using (var context = await TestCommon.Instance.GetContextAsync(TestCommon.TestSite))
            {
                var me = context.Me.Get(o => o.Chats);
                Assert.IsTrue(me.Chats.Length > 0);

                var chat = me.Chats.AsRequested().FirstOrDefault(i => i.ChatType == ChatTypeConstants.OneOnOne);
                Assert.IsNotNull(chat);

                chat.Load(o => o.Messages);
                var chatMessages = chat.Messages;

                Assert.IsNotNull(chatMessages);

                // assume as if there are no chat messages
                // There appears to be no remove option yet in this feature - so add a recognisable message
                var body = $"Hello, This is a unit test (AddChatMessageSubjectOneOnOneAsyncTest) posting a message - PnP Rocks! - Woah...";
                await chatMessages.AddAsync(body, subject: "This is a subject test");

                chat = await chat.GetAsync(o => o.Messages);
                var updateMessages = chat.Messages.AsEnumerable();

                var message = updateMessages.Last();
                Assert.IsNotNull(message.CreatedDateTime);
                // Depending on regional settings this check might fail
                //Assert.AreEqual(message.DeletedDateTime, DateTime.MinValue);
                Assert.IsNotNull(message.Etag);
                Assert.IsNotNull(message.Importance);
                Assert.IsNotNull(message.LastModifiedDateTime);
                Assert.IsNotNull(message.Locale);
                Assert.IsNotNull(message.MessageType);

                Assert.IsTrue(message.IsPropertyAvailable(o => o.ReplyToId));
                Assert.IsNull(message.ReplyToId);
                // BERT - Check with PAUL
                //Assert.IsTrue(message.IsPropertyAvailable(o => o.Subject));
                //Assert.IsNotNull(message.Subject);
                Assert.IsTrue(message.IsPropertyAvailable(o => o.Summary));
                Assert.IsNull(message.Summary);

            }
        }

        [TestMethod]
        public async Task AddChatMessageThumbnailOneOnOneAsyncTest()
        {
            //TestCommon.Instance.Mocking = false;
            using (var context = await TestCommon.Instance.GetContextAsync(TestCommon.TestSite))
            {
                var me = context.Me.Get(o => o.Chats);
                Assert.IsTrue(me.Chats.Length > 0);

                var chat = me.Chats.AsRequested().FirstOrDefault(i => i.ChatType == ChatTypeConstants.OneOnOne);
                Assert.IsNotNull(chat);

                chat.Load(o => o.Messages);
                var chatMessages = chat.Messages;

                Assert.IsNotNull(chatMessages);

                // assume as if there are no chat messages
                // There appears to be no remove option yet in this feature - so add a recognisable message
                var attachmentId = "74d20c7f34aa4a7fb74e2b30004247c5";
                var body = $"<attachment id=\"{attachmentId}\"></attachment>";

                await chatMessages.AddAsync(new ChatMessageOptions
                {
                    Content = body,
                    ContentType = ChatMessageContentType.Html,
                    Attachments =
                        {
                             new ChatMessageAttachmentOptions
                            {
                                Id = attachmentId,
                                ContentType = "application/vnd.microsoft.card.thumbnail",
                                // Adaptive Card
                                Content = "{\r\n  \"title\": \"Unit Test posting a card\",\r\n  \"subtitle\": \"<h3>This is the subtitle</h3>\",\r\n  \"text\": \"Here is some body text. <br>\\r\\nAnd a <a href=\\\"http://microsoft.com/\\\">hyperlink</a>. <br>\\r\\nAnd below that is some buttons:\",\r\n  \"buttons\": [\r\n    {\r\n      \"type\": \"messageBack\",\r\n      \"title\": \"Login to FakeBot\",\r\n      \"text\": \"login\",\r\n      \"displayText\": \"login\",\r\n      \"value\": \"login\"\r\n    }\r\n  ]\r\n}",
                                ContentUrl = null,
                                Name = null,
                                ThumbnailUrl = null
                            }
                        }
                });

                chat = await chat.GetAsync(o => o.Messages);
                var updateMessages = chat.Messages.AsRequested();

                var message = updateMessages.Last();
                Assert.IsNotNull(message.CreatedDateTime);
                // Depending on regional settings this check might fail
                //Assert.AreEqual(message.DeletedDateTime, DateTime.MinValue);
                Assert.IsNotNull(message.Etag);
                Assert.IsNotNull(message.Importance);
                Assert.IsNotNull(message.LastModifiedDateTime);
                Assert.IsNotNull(message.Locale);
                Assert.IsNotNull(message.MessageType);

                Assert.IsTrue(message.IsPropertyAvailable(o => o.ReplyToId));
                Assert.IsNull(message.ReplyToId);
                Assert.IsTrue(message.IsPropertyAvailable(o => o.Subject));
                Assert.IsNull(message.Subject);
                Assert.IsTrue(message.IsPropertyAvailable(o => o.Summary));
                Assert.IsNull(message.Summary);
            }
        }

        [TestMethod]
        public async Task AddChatMessageAdaptiveOneOnOneAsyncTest()
        {
            //Reference: https://docs.microsoft.com/en-us/microsoftteams/platform/task-modules-and-cards/cards/cards-reference#adaptive-card

            //TestCommon.Instance.Mocking = false;
            using (var context = await TestCommon.Instance.GetContextAsync(TestCommon.TestSite))
            {
                var me = context.Me.Get(o => o.Chats);
                Assert.IsTrue(me.Chats.Length > 0);

                var chat = me.Chats.AsRequested().FirstOrDefault(i => i.ChatType == ChatTypeConstants.OneOnOne);
                Assert.IsNotNull(chat);

                chat.Load(o => o.Messages);
                var chatMessages = chat.Messages;

                Assert.IsNotNull(chatMessages);

                // assume as if there are no chat messages
                // There appears to be no remove option yet in this feature - so add a recognisable message
                var attachmentId = "74d20c7f34aa4a7fb74e2b30004247c5";
                var body = $"<attachment id=\"{attachmentId}\"></attachment>";

                await chatMessages.AddAsync(new ChatMessageOptions
                {
                    Content = body,
                    ContentType = ChatMessageContentType.Html,
                    Attachments = {
                        new ChatMessageAttachmentOptions
                        {
                            Id = attachmentId,
                            ContentType = "application/vnd.microsoft.card.adaptive",
                            // Adaptive Card
                            Content = "{\"$schema\":\"http://adaptivecards.io/schemas/adaptive-card.json\",\"type\":\"AdaptiveCard\",\"version\":\"1.0\",\"body\":[{\"type\":\"Container\",\"items\":[{\"type\":\"TextBlock\",\"text\":\"Adaptive Card Unit Test\",\"weight\":\"bolder\",\"size\":\"medium\"},{\"type\":\"ColumnSet\",\"columns\":[{\"type\":\"Column\",\"width\":\"auto\",\"items\":[{\"type\":\"Image\",\"url\":\"https://pbs.twimg.com/profile_images/3647943215/d7f12830b3c17a5a9e4afcc370e3a37e_400x400.jpeg\",\"size\":\"small\",\"style\":\"person\"}]},{\"type\":\"Column\",\"width\":\"stretch\",\"items\":[{\"type\":\"TextBlock\",\"text\":\"Matt Hidinger\",\"weight\":\"bolder\",\"wrap\":true},{\"type\":\"TextBlock\",\"spacing\":\"none\",\"text\":\"Created {{DATE(2017-02-14T06:08:39Z,SHORT)}}\",\"isSubtle\":true,\"wrap\":true}]}]}]},{\"type\":\"Container\",\"items\":[{\"type\":\"TextBlock\",\"text\":\"Now that we have defined the main rule sand features of the format ,we need to produce a schema and publish it to GitHub.The schema will be the starting point of our reference documentation.\",\"wrap\":true},{\"type\":\"FactSet\",\"facts\":[{\"title\":\"Board:\",\"value\":\"Adaptive Card\"},{\"title\":\"List:\",\"value\":\"Backlog\"},{\"title\":\"Assigned to:\",\"value\":\"Matt Hidinger\"},{\"title\":\"Duedate:\",\"value\":\"Not set\"}]}]}],\"actions\":[{\"type\":\"Action.ShowCard\",\"title\":\"Set due date\",\"card\":{\"type\":\"AdaptiveCard\",\"body\":[{\"type\":\"Input.Date\",\"id\":\"dueDate\"}],\"actions\":[{\"type\":\"Action.Submit\",\"title\":\"OK\"}]}},{\"type\":\"Action.ShowCard\",\"title\":\"Comment\",\"card\":{\"type\":\"AdaptiveCard\",\"body\":[{\"type\":\"Input.Text\",\"id\":\"comment\",\"isMultiline\":true,\"placeholder\":\"Enter your comment\"}],\"actions\":[{\"type\":\"Action.Submit\",\"title\":\"OK\"}]}}]}",
                            ContentUrl = null,
                            Name = null,
                            ThumbnailUrl = null
                        }
                    }
                });

                chat = await chat.GetAsync(o => o.Messages);
                var updateMessages = chat.Messages.AsEnumerable();

                var message = updateMessages.Last();
                Assert.IsNotNull(message.CreatedDateTime);
                Assert.IsNotNull(message.Etag);
                Assert.IsNotNull(message.Importance);
                Assert.IsNotNull(message.LastModifiedDateTime);
                Assert.IsNotNull(message.Locale);
                Assert.IsNotNull(message.MessageType);

                Assert.IsTrue(message.IsPropertyAvailable(o => o.ReplyToId));
                Assert.IsNull(message.ReplyToId);
                Assert.IsTrue(message.IsPropertyAvailable(o => o.Subject));
                Assert.IsNull(message.Subject);
                Assert.IsTrue(message.IsPropertyAvailable(o => o.Summary));
                Assert.IsNull(message.Summary);
            }
        }

        [TestMethod]
        public void AddChatMessageOneOnOneBatchTest()
        {
            //TestCommon.Instance.Mocking = false;
            using (var context = TestCommon.Instance.GetContext(TestCommon.TestSite))
            {
                var batch = context.NewBatch();

                var me = context.Me.GetBatch(batch, o => o.Chats);
                context.Execute(batch);

                var primaryChat = me.Result.Chats.AsRequested().FirstOrDefault(i => i.ChatType == ChatTypeConstants.OneOnOne);
                Assert.IsNotNull(primaryChat);

                batch = context.NewBatch();
                var chat = primaryChat.GetBatch(batch, o => o.Messages);
                context.Execute(batch);

                var chatMessages = chat.Result.Messages;
                Assert.IsNotNull(chatMessages);

                // assume as if there are no chat messages
                // There appears to be no remove option yet in this feature - so add a recognisable message
                var body = $"Hello, this is a unit test (AddChatMessageOneOnOneBatchTest) posting a message - PnP Rocks! - Woah...";

                chatMessages.AddBatch(body, ChatMessageContentType.Text, "Batch Test");
                context.Execute();

                chat = primaryChat.GetBatch(o => o.Messages);
                context.Execute();
                var updateMessages = chat.Result.Messages;

                var message = updateMessages.AsRequested().FirstOrDefault(o => o.Body.Content == body);

                Assert.IsFalse(message == default);
                Assert.IsNotNull(message.CreatedDateTime);
                Assert.IsNotNull(message.Etag);
                Assert.IsNotNull(message.Importance);
                Assert.IsNotNull(message.LastModifiedDateTime);
                Assert.IsNotNull(message.Locale);
                Assert.IsNotNull(message.MessageType);

                Assert.IsTrue(message.IsPropertyAvailable(o => o.ReplyToId));
                Assert.IsNull(message.ReplyToId);
                Assert.IsTrue(message.IsPropertyAvailable(o => o.Subject));
                Assert.IsNotNull(message.Subject);
                Assert.IsTrue(message.IsPropertyAvailable(o => o.Summary));
                Assert.IsNull(message.Summary);

            }
        }

        [TestMethod]
        public async Task AddChatMessageBatchOneOnOneAsyncTest()
        {
            //TestCommon.Instance.Mocking = false;
            using (var context = TestCommon.Instance.GetContext(TestCommon.TestSite))
            {
                var me = await context.Me.GetBatchAsync(o => o.Chats);
                await context.ExecuteAsync();
                Assert.IsTrue(me.Result.Chats.Length > 0);

                var chatQuery = me.Result.Chats.FirstOrDefault(i => i.ChatType == ChatTypeConstants.OneOnOne);
                Assert.IsNotNull(chatQuery);

                var chat = await chatQuery.GetBatchAsync(o => o.Messages);
                await context.ExecuteAsync();
                var chatMessages = chat.Result.Messages;

                Assert.IsNotNull(chatMessages);

                // assume as if there are no chat messages
                // There appears to be no remove option yet in this feature - so add a recognisable message
                var body = $"Hello, this is a unit test (AddChatMessageBatchOneOnOneAsyncTest) posting a message - PnP Rocks! - Woah...";
                await chatMessages.AddBatchAsync(body, ChatMessageContentType.Text, "Batch Test");
                await context.ExecuteAsync();

                chat = chatQuery.GetBatch(o => o.Messages);
                await context.ExecuteAsync();
                var updateMessages = chat.Result.Messages;

                var message = updateMessages.AsRequested().FirstOrDefault(o => o.Body.Content == body);

                Assert.IsFalse(message == default);
                Assert.IsNotNull(message.CreatedDateTime);
                // Depending on regional settings this check might fail
                //Assert.AreEqual(message.DeletedDateTime, DateTime.MinValue);
                Assert.IsNotNull(message.Etag);
                Assert.IsNotNull(message.Importance);
                Assert.IsNotNull(message.LastModifiedDateTime);
                Assert.IsNotNull(message.Locale);
                Assert.IsNotNull(message.MessageType);

                Assert.IsTrue(message.IsPropertyAvailable(o => o.ReplyToId));
                Assert.IsNull(message.ReplyToId);
                Assert.IsTrue(message.IsPropertyAvailable(o => o.Subject));
                Assert.IsNotNull(message.Subject);
                Assert.IsTrue(message.IsPropertyAvailable(o => o.Summary));
                Assert.IsNull(message.Summary);

            }
        }

        [TestMethod]
        public void AddChatMessageBatchOptionsOneOnOneTest()
        {
            //TestCommon.Instance.Mocking = false;
            using (var context = TestCommon.Instance.GetContext(TestCommon.TestSite))
            {
                var me = context.Me.GetBatch(o => o.Chats);
                context.Execute();
                Assert.IsTrue(me.Result.Chats.Length > 0);

                var chatQuery = me.Result.Chats.FirstOrDefault(i => i.ChatType == ChatTypeConstants.OneOnOne);
                Assert.IsNotNull(chatQuery);

                var chat = chatQuery.GetBatch(o => o.Messages);
                context.Execute();
                var chatMessages = chat.Result.Messages;

                Assert.IsNotNull(chatMessages);

                // assume as if there are no chat messages
                // There appears to be no remove option yet in this feature - so add a recognisable message
                var body = $"Hello, this is a unit test (AddChatMessageBatchOptionsOneOnOneTest) posting a message - PnP Rocks! - Woah...";

                chatMessages.AddBatch(new ChatMessageOptions()
                {
                    Content = body,
                    Subject = "ChatMessageBatchOptionsTest"
                });
                context.Execute();


                chat = chatQuery.GetBatch(o => o.Messages);
                context.Execute();
                var updateMessages = chat.Result.Messages;

                var message = updateMessages.AsRequested().FirstOrDefault(o => o.Body.Content == body);

                Assert.IsFalse(message == default);
                Assert.IsNotNull(message.CreatedDateTime);
                // Depending on regional settings this check might fail
                //Assert.AreEqual(message.DeletedDateTime, DateTime.MinValue);
                Assert.IsNotNull(message.Etag);
                Assert.IsNotNull(message.Importance);
                Assert.IsNotNull(message.LastModifiedDateTime);
                Assert.IsNotNull(message.Locale);
                Assert.IsNotNull(message.MessageType);

                Assert.IsTrue(message.IsPropertyAvailable(o => o.ReplyToId));
                Assert.IsNull(message.ReplyToId);
                Assert.IsTrue(message.IsPropertyAvailable(o => o.Subject));
                Assert.IsNotNull(message.Subject);
                Assert.IsTrue(message.IsPropertyAvailable(o => o.Summary));
                Assert.IsNull(message.Summary);

            }
        }

        [TestMethod]
        public async Task AddChatMessageBatchOptionsOneOnOneAsyncTest()
        {
            //TestCommon.Instance.Mocking = false;
            using (var context = TestCommon.Instance.GetContext(TestCommon.TestSite))
            {
                var me = await context.Me.GetBatchAsync(o => o.Chats);
                await context.ExecuteAsync();
                Assert.IsTrue(me.Result.Chats.Length > 0);

                var chatQuery = me.Result.Chats.FirstOrDefault(i => i.ChatType == ChatTypeConstants.OneOnOne);
                Assert.IsNotNull(chatQuery);

                var chat = await chatQuery.GetBatchAsync(o => o.Messages);
                await context.ExecuteAsync();
                var chatMessages = chat.Result.Messages;

                Assert.IsNotNull(chatMessages);

                // assume as if there are no chat messages
                // There appears to be no remove option yet in this feature - so add a recognisable message
                var body = $"Hello, this is a unit test (AddChatMessageBatchOptionsOneOnOneAsyncTest) posting a message - PnP Rocks! - Woah...";
                await chatMessages.AddBatchAsync(new ChatMessageOptions()
                {
                    Content = body,
                    Subject = "AddChatMessageBatchOptionsAsyncTest"
                });
                await context.ExecuteAsync();

                chat = chatQuery.GetBatch(o => o.Messages);
                await context.ExecuteAsync();
                var updateMessages = chat.Result.Messages;

                var message = updateMessages.AsRequested().FirstOrDefault(o => o.Body.Content == body);

                Assert.IsFalse(message == default);
                Assert.IsNotNull(message.CreatedDateTime);
                // Depending on regional settings this check might fail
                //Assert.AreEqual(message.DeletedDateTime, DateTime.MinValue);
                Assert.IsNotNull(message.Etag);
                Assert.IsNotNull(message.Importance);
                Assert.IsNotNull(message.LastModifiedDateTime);
                Assert.IsNotNull(message.Locale);
                Assert.IsNotNull(message.MessageType);

                Assert.IsTrue(message.IsPropertyAvailable(o => o.ReplyToId));
                Assert.IsNull(message.ReplyToId);
                Assert.IsTrue(message.IsPropertyAvailable(o => o.Subject));
                Assert.IsNotNull(message.Subject);
                Assert.IsTrue(message.IsPropertyAvailable(o => o.Summary));
                Assert.IsNull(message.Summary);

            }
        }

        [TestMethod]
        public void AddChatMessageSpecificOneOnOneBatchTest()
        {
            //TestCommon.Instance.Mocking = false;
            using (var context = TestCommon.Instance.GetContext(TestCommon.TestSite))
            {
                var batch = context.NewBatch();
                var me = context.Me.GetBatch(batch, o => o.Chats);
                context.Execute(batch);
                Assert.IsTrue(me.Result.Chats.Length > 0);

                var chatQuery = me.Result.Chats.FirstOrDefault(i => i.ChatType == ChatTypeConstants.OneOnOne);
                Assert.IsNotNull(chatQuery);

                var chat = chatQuery.GetBatch(batch, o => o.Messages);
                context.Execute(batch);
                var chatMessages = chat.Result.Messages;

                Assert.IsNotNull(chatMessages);

                // assume as if there are no chat messages
                // There appears to be no remove option yet in this feature - so add a recognisable message
                var body = $"Hello, this is a unit test (AddChatMessageSpecificOneOnOneBatchTest) posting a message - PnP Rocks! - Woah...";
                chatMessages.AddBatch(batch, body);
                context.Execute(batch);

                var batch2 = context.NewBatch();
                chat = chatQuery.GetBatch(batch2, o => o.Messages);
                context.Execute(batch2);
                var updateMessages = chat.Result.Messages.AsRequested();

                var message = updateMessages.First(o => o.Body.Content == body);
                Assert.IsNotNull(message.CreatedDateTime);

                // Depending on regional settings this check might fail
                //Assert.AreEqual(message.DeletedDateTime, DateTime.MinValue);
                Assert.IsNotNull(message.Etag);
                Assert.IsNotNull(message.Importance);
                Assert.IsNotNull(message.LastModifiedDateTime);
                Assert.IsNotNull(message.Locale);
                Assert.IsNotNull(message.MessageType);

                Assert.IsTrue(message.IsPropertyAvailable(o => o.ReplyToId));
                Assert.IsNull(message.ReplyToId);
                Assert.IsTrue(message.IsPropertyAvailable(o => o.Subject));
                Assert.IsNull(message.Subject);
                Assert.IsTrue(message.IsPropertyAvailable(o => o.Summary));
                Assert.IsNull(message.Summary);
            }
        }

        [TestMethod]
        public async Task AddChatMessageSpecificBatchOneOnOneAsyncTest()
        {
            //TestCommon.Instance.Mocking = false;
            using (var context = TestCommon.Instance.GetContext(TestCommon.TestSite))
            {
                var batch = context.NewBatch();
                var me = await context.Me.GetBatchAsync(batch, o => o.Chats);
                await context.ExecuteAsync(batch);
                Assert.IsTrue(me.Result.Chats.Length > 0);

                var chatQuery = me.Result.Chats.FirstOrDefault(i => i.ChatType == ChatTypeConstants.OneOnOne);
                Assert.IsNotNull(chatQuery);

                var chat = await chatQuery.GetBatchAsync(batch, o => o.Messages);
                await context.ExecuteAsync(batch);
                var chatMessages = chat.Result.Messages;

                Assert.IsNotNull(chatMessages);

                // assume as if there are no chat messages
                // There appears to be no remove option yet in this feature - so add a recognisable message
                var body = $"Hello, this is a unit test (AddChatMessageSpecificBatchAsyncTest) posting a message - PnP Rocks! - Woah...";
                await chatMessages.AddBatchAsync(batch, body, ChatMessageContentType.Text, "Batch Async Test");
                await context.ExecuteAsync(batch);

                var batch2 = context.NewBatch();
                chat = await chatQuery.GetBatchAsync(batch2, o => o.Messages);
                await context.ExecuteAsync(batch2);
                var updateMessages = chat.Result.Messages.AsRequested();

                var message = updateMessages.First(o => o.Body.Content == body);
                Assert.IsNotNull(message.CreatedDateTime);

                // Depending on regional settings this check might fail
                //Assert.AreEqual(message.DeletedDateTime, DateTime.MinValue);
                Assert.IsNotNull(message.Etag);
                Assert.IsNotNull(message.Importance);
                Assert.IsNotNull(message.LastModifiedDateTime);
                Assert.IsNotNull(message.Locale);
                Assert.IsNotNull(message.MessageType);

                Assert.IsTrue(message.IsPropertyAvailable(o => o.ReplyToId));
                Assert.IsNull(message.ReplyToId);
                Assert.IsTrue(message.IsPropertyAvailable(o => o.Subject));
                Assert.IsNotNull(message.Subject);
                Assert.IsTrue(message.IsPropertyAvailable(o => o.Summary));
                Assert.IsNull(message.Summary);
            }
        }

        [TestMethod]
        [ExpectedException(typeof(ArgumentNullException))]
        public void AddChatMessageBatchOptionsOneOnOneExceptionTest()
        {
            //TestCommon.Instance.Mocking = false;
            using (var context = TestCommon.Instance.GetContext(TestCommon.TestSite))
            {
                var batch = context.NewBatch();
                var me = context.Me.GetBatch(batch, o => o.Chats);
                context.Execute(batch);
                Assert.IsTrue(me.Result.Chats.Length > 0);

                var chatQuery = me.Result.Chats.FirstOrDefault(i => i.ChatType == ChatTypeConstants.OneOnOne);
                Assert.IsNotNull(chatQuery);

                var chat = chatQuery.GetBatch(batch, o => o.Messages);
                context.Execute(batch);
                var chatMessages = chat.Result.Messages;

                Assert.IsNotNull(chatMessages);

                // assume as if there are no chat messages
                // There appears to be no remove option yet in this feature - so add a recognisable message
                var body = string.Empty;
                chatMessages.AddBatch(batch, options: null);
                context.Execute(batch);

            }
        }

        [TestMethod]
        [ExpectedException(typeof(ArgumentNullException))]
        public void AddChatMessageBatchOneOnOneExceptionTest()
        {
            //TestCommon.Instance.Mocking = false;
            using (var context = TestCommon.Instance.GetContext(TestCommon.TestSite))
            {
                var batch = context.NewBatch();
                var me = context.Me.GetBatch(batch, o => o.Chats);
                context.Execute(batch);
                Assert.IsTrue(me.Result.Chats.Length > 0);

                var chatQuery = me.Result.Chats.FirstOrDefault(i => i.ChatType == ChatTypeConstants.OneOnOne);
                Assert.IsNotNull(chatQuery);

                var chat = chatQuery.GetBatch(batch, o => o.Messages);
                context.Execute(batch);
                var chatMessages = chat.Result.Messages;

                Assert.IsNotNull(chatMessages);

                // assume as if there are no chat messages
                // There appears to be no remove option yet in this feature - so add a recognisable message
                var body = string.Empty;
                chatMessages.AddBatch(batch, body);
                context.Execute(batch);

            }
        }

        [TestMethod]
        [ExpectedException(typeof(ArgumentNullException))]
        public void AddChatMessageOneOnOneExceptionTest()
        {
            //TestCommon.Instance.Mocking = false;
            using (var context = TestCommon.Instance.GetContext(TestCommon.TestSite))
            {
                var me = context.Me.Get(o => o.Chats);
                Assert.IsTrue(me.Chats.Length > 0);

                var chat = me.Chats.AsRequested().FirstOrDefault(i => i.ChatType == ChatTypeConstants.OneOnOne);
                Assert.IsNotNull(chat);

                chat.Load(o => o.Messages);
                var chatMessages = chat.Messages;

                Assert.IsNotNull(chatMessages);

                // assume as if there are no chat messages
                // There appears to be no remove option yet in this feature - so add a recognisable message
                var body = string.Empty;
                chatMessages.Add(body);
            }
        }

        [TestMethod]
        public async Task GetChatMessageGroupAsyncTest()
        {
            //TestCommon.Instance.Mocking = false;
            using (var context = await TestCommon.Instance.GetContextAsync(TestCommon.TestSite))
            {
                var me = await context.Me.GetAsync(o => o.Chats);
                Assert.IsTrue(me.Chats.Length > 0);

                var chat = me.Chats.AsRequested().FirstOrDefault(i => i.ChatType == ChatTypeConstants.Group && i.Topic == GroupChatTopic);
                Assert.IsNotNull(chat);

                chat = await chat.GetAsync(o => o.Messages);
                var chatMessages = chat.Messages;

                Assert.IsNotNull(chatMessages);

                // assume as if there are no chat messages
                // There appears to be no remove option yet in this feature - so add a recognisable message
                var body = "Hello, this is a unit test (GetChatMessageGroupAsyncTest) posting a message - PnP Rocks!";
                await chatMessages.AddAsync(body);

                chat = await chat.GetAsync(o => o.Messages);
                var updateMessages = chat.Messages.AsRequested();

                var message = updateMessages.First(o => o.Body.Content == body);
                Assert.IsNotNull(message.CreatedDateTime);
                Assert.IsNotNull(message.Etag);
                Assert.IsNotNull(message.Importance);
                Assert.IsNotNull(message.LastModifiedDateTime);
                Assert.IsNotNull(message.Locale);
                Assert.IsNotNull(message.MessageType);

                Assert.IsTrue(message.IsPropertyAvailable(o => o.ReplyToId));
                Assert.IsNull(message.ReplyToId);
                Assert.IsTrue(message.IsPropertyAvailable(o => o.Subject));
                Assert.IsNull(message.Subject);
                Assert.IsTrue(message.IsPropertyAvailable(o => o.Summary));
                Assert.IsNull(message.Summary);
            }
        }


        [TestMethod]
        public void AddChatMessageToGroupChatTest()
        {
            //TestCommon.Instance.Mocking = false;
            using (var context = TestCommon.Instance.GetContext(TestCommon.TestSite))
            {
                var me = context.Me.Get(o => o.Chats);
                Assert.IsTrue(me.Chats.Length > 0);

                var chat = me.Chats.AsRequested().FirstOrDefault(i => i.ChatType == ChatTypeConstants.Group && i.Topic == GroupChatTopic);
                Assert.IsNotNull(chat);

                chat.Load(o => o.Messages);
                var chatMessages = chat.Messages;

                Assert.IsNotNull(chatMessages);

                // assume as if there are no chat messages
                // There appears to be no remove option yet in this feature - so add a recognisable message
                var body = $"Hello, this is a unit test (AddChatMessageToGroupChatTest) posting a message - PnP Rocks! - Woah...";
                chatMessages.Add(body);

                chat = chat.Get(o => o.Messages);
                var updateMessages = chat.Messages.AsRequested();

                var message = updateMessages.First(o => o.Body.Content == body);
                Assert.IsNotNull(message.CreatedDateTime);
                Assert.IsNotNull(message.Etag);
                Assert.IsNotNull(message.Importance);
                Assert.IsNotNull(message.LastModifiedDateTime);
                Assert.IsNotNull(message.Locale);
                Assert.IsNotNull(message.MessageType);

                Assert.IsTrue(message.IsPropertyAvailable(o => o.ReplyToId));
                Assert.IsNull(message.ReplyToId);
                Assert.IsTrue(message.IsPropertyAvailable(o => o.Subject));
                Assert.IsNull(message.Subject);
                Assert.IsTrue(message.IsPropertyAvailable(o => o.Summary));
                Assert.IsNull(message.Summary);
            }
        }

        [TestMethod]
        public void AddChatMessageOptionsGroupTest()
        {
            //TestCommon.Instance.Mocking = false;
            using (var context = TestCommon.Instance.GetContext(TestCommon.TestSite))
            {
                var me = context.Me.Get(o => o.Chats);
                Assert.IsTrue(me.Chats.Length > 0);

                var chat = me.Chats.AsRequested().FirstOrDefault(i => i.ChatType == ChatTypeConstants.Group && i.Topic == GroupChatTopic);
                Assert.IsNotNull(chat);

                chat.Load(o => o.Messages);
                var chatMessages = chat.Messages;

                Assert.IsNotNull(chatMessages);

                // assume as if there are no chat messages
                // There appears to be no remove option yet in this feature - so add a recognisable message
                var body = $"Hello, this is a unit test (AddChatMessageOptionsGroupTest) posting a message - PnP Rocks! - Woah...";
                var subject = "Options";
                chatMessages.Add(new ChatMessageOptions()
                {
                    Content = body,
                    Subject = subject
                });

                chat = chat.Get(o => o.Messages);
                var updateMessages = chat.Messages.AsRequested();

                var message = updateMessages.First(o => o.Body.Content == body);
                Assert.IsNotNull(message.CreatedDateTime);
                Assert.IsNotNull(message.Etag);
                Assert.IsNotNull(message.Importance);
                Assert.IsNotNull(message.LastModifiedDateTime);
                Assert.IsNotNull(message.Locale);
                Assert.IsNotNull(message.MessageType);

                Assert.IsTrue(message.IsPropertyAvailable(o => o.ReplyToId));
                Assert.IsNull(message.ReplyToId);
                Assert.IsTrue(message.IsPropertyAvailable(o => o.Subject));
                Assert.IsNotNull(message.Subject);
                Assert.IsTrue(message.IsPropertyAvailable(o => o.Summary));
                Assert.IsNull(message.Summary);
            }
        }

        [TestMethod]
        public async Task AddChatMessageHtmlGroupAsyncTest()
        {
            //TestCommon.Instance.Mocking = false;
            using (var context = await TestCommon.Instance.GetContextAsync(TestCommon.TestSite))
            {
                var me = context.Me.Get(o => o.Chats);
                Assert.IsTrue(me.Chats.Length > 0);

                var chat = me.Chats.AsRequested().FirstOrDefault(i => i.ChatType == ChatTypeConstants.Group && i.Topic == GroupChatTopic);
                Assert.IsNotNull(chat);

                chat.Load(o => o.Messages);
                var chatMessages = chat.Messages;

                Assert.IsNotNull(chatMessages);

                // assume as if there are no chat messages
                // There appears to be no remove option yet in this feature - so add a recognisable message
                var body = $"<h1>Hello</h1><br />This is a unit test (AddChatMessageHtmlGroupAsyncTest) posting a message - <strong>PnP Rocks!</strong> - Woah...";

                await chatMessages.AddAsync(body, ChatMessageContentType.Html);

                chat = await chat.GetAsync(o => o.Messages);
                var updateMessages = chat.Messages;

                var message = updateMessages.AsEnumerable().Last();
                Assert.IsNotNull(message.CreatedDateTime);
                // Depending on regional settings this check might fail
                //Assert.AreEqual(message.DeletedDateTime, DateTime.MinValue);
                Assert.IsNotNull(message.Etag);
                Assert.IsNotNull(message.Importance);
                Assert.IsNotNull(message.LastModifiedDateTime);
                Assert.IsNotNull(message.Locale);
                Assert.IsNotNull(message.MessageType);

                Assert.IsTrue(message.IsPropertyAvailable(o => o.ReplyToId));
                Assert.IsNull(message.ReplyToId);
                Assert.IsTrue(message.IsPropertyAvailable(o => o.Subject));
                Assert.IsNull(message.Subject);
                Assert.IsTrue(message.IsPropertyAvailable(o => o.Summary));
                Assert.IsNull(message.Summary);

            }
        }

        [TestMethod]
        public async Task AddChatMessageFileAttachmentGroupAsyncTest()
        {
            //TestCommon.Instance.Mocking = false;
            using (var context = await TestCommon.Instance.GetContextAsync(TestCommon.TestSite))
            {
                var me = context.Me.Get(o => o.Chats);
                Assert.IsTrue(me.Chats.Length > 0);

                var chat = me.Chats.AsRequested().FirstOrDefault(i => i.ChatType == ChatTypeConstants.Group && i.Topic == GroupChatTopic);
                Assert.IsNotNull(chat);

                chat.Load(o => o.Messages);
                var chatMessages = chat.Messages;

                Assert.IsNotNull(chatMessages);

                // Upload File to SharePoint Library - it will have to remain i guess as onetime upload.
                IFolder folder = await context.Web.Lists.GetByTitle("Documents").RootFolder.GetAsync();
                IFile existingFile = await folder.Files.FirstOrDefaultAsync(o => o.Name == "test_added.docx");
                if (existingFile == default)
                {
                    existingFile = await folder.Files.AddAsync("test_added.docx", System.IO.File.OpenRead($".{Path.DirectorySeparatorChar}TestAssets{Path.DirectorySeparatorChar}test.docx"));
                }

                Assert.IsNotNull(existingFile);
                Assert.AreEqual("test_added.docx", existingFile.Name);

                // Useful reference - https://docs.microsoft.com/en-us/graph/api/chatmessage-post?view=graph-rest-beta&tabs=http#example-4-file-attachments
                // assume as if there are no chat messages
                var attachmentId = existingFile.ETag.AsGraphEtag(); // Needs to be the documents eTag - just the GUID part
                var body = $"<h1>Hello</h1><br />This is a unit test with a file attachment (AddChatMessageFileAttachmentGroupAsyncTest) posting a message - <attachment id=\"{attachmentId}\"></attachment>";

                var fileUri = new Uri(existingFile.LinkingUrl);

                await chatMessages.AddAsync(new ChatMessageOptions
                {
                    Content = body,
                    ContentType = ChatMessageContentType.Html,
                    Attachments = {
                        new ChatMessageAttachmentOptions
                        {
                            Id = attachmentId,
                            ContentType = "reference",
                            // Cannot have the extension with a query graph doesnt recognise and think its part of file extension - include in docs.
                            ContentUrl = new Uri(fileUri.ToString().Replace(fileUri.Query, "")),
                            Name = $"{existingFile.Name}",
                            ThumbnailUrl = null,
                            Content = null
                        }
                    }
                });

                chat = await chat.GetAsync(o => o.Messages);
                var updateMessages = chat.Messages;

                var message = updateMessages.AsEnumerable().Last();
                Assert.IsNotNull(message.CreatedDateTime);
                Assert.IsNotNull(message.Etag);
                Assert.IsNotNull(message.Importance);
                Assert.IsNotNull(message.LastModifiedDateTime);
                Assert.IsNotNull(message.Locale);
                Assert.IsNotNull(message.MessageType);

                Assert.IsTrue(message.IsPropertyAvailable(o => o.ReplyToId));
                Assert.IsNull(message.ReplyToId);
                Assert.IsTrue(message.IsPropertyAvailable(o => o.Subject));
                Assert.IsNull(message.Subject);
                Assert.IsTrue(message.IsPropertyAvailable(o => o.Summary));
                Assert.IsNull(message.Summary);

                //delete file
                await existingFile.DeleteAsync(); //Note this will break the link in the Teams chat.
            }
        }

        [TestMethod]
        public async Task AddChatMessageInlineImagesGroupAsyncTest()
        {
            //TestCommon.Instance.Mocking = false;
            using (var context = await TestCommon.Instance.GetContextAsync(TestCommon.TestSite))
            {
                var me = context.Me.Get(o => o.Chats);
                Assert.IsTrue(me.Chats.Length > 0);

                var chat = me.Chats.AsRequested().FirstOrDefault(i => i.ChatType == ChatTypeConstants.Group && i.Topic == GroupChatTopic);
                Assert.IsNotNull(chat);

                chat.Load(o => o.Messages);
                var chatMessages = chat.Messages;

                Assert.IsNotNull(chatMessages);

                // Useful reference - https://docs.microsoft.com/en-us/graph/api/chatmessage-post?view=graph-rest-beta&tabs=http#example-5-sending-inline-images-along-with-the-message
                // assume as if there are no chat messages

                var body = $"<div><div><h1>Hello</h1><p>This is a unit test (AddChatMessageInlineImagesGroupAsyncTest) posting a message with inline image</p><div><span><img height=\"392\" src=\"../hostedContents/1/$value\" width=\"300\" style=\"vertical-align:bottom; width:300px; height:392px\"></span></div></div></div>";

                await chatMessages.AddAsync(new ChatMessageOptions
                {
                    Content = body,
                    ContentType = ChatMessageContentType.Html,
                    HostedContents =
                    {
                        new ChatMessageHostedContentOptions
                        {
                            Id = "1",
                            ContentBytes = "",
                            ContentType = "image/png"
                        }
                    }
                });

                chat = await chat.GetAsync(o => o.Messages);
                var updateMessages = chat.Messages;

                var message = updateMessages.AsEnumerable().Last();
                Assert.IsNotNull(message.CreatedDateTime);
                // Depending on regional settings this check might fail
                //Assert.AreEqual(message.DeletedDateTime, DateTime.MinValue);
                Assert.IsNotNull(message.Etag);
                Assert.IsNotNull(message.Importance);
                Assert.IsNotNull(message.LastModifiedDateTime);
                Assert.IsNotNull(message.Locale);
                Assert.IsNotNull(message.MessageType);
                Assert.IsNotNull(message.HostedContents);

                Assert.IsTrue(message.IsPropertyAvailable(o => o.ReplyToId));
                Assert.IsNull(message.ReplyToId);
                Assert.IsTrue(message.IsPropertyAvailable(o => o.Subject));
                Assert.IsNull(message.Subject);
                Assert.IsTrue(message.IsPropertyAvailable(o => o.Summary));
                Assert.IsNull(message.Summary);

            }
        }

        [TestMethod]
        public void AddChatMessageInlineImagesGroupBatchAsyncTest()
        {
            //TestCommon.Instance.Mocking = false;
            using (var context = TestCommon.Instance.GetContext(TestCommon.TestSite))
            {
                var batch = context.NewBatch();

                var me = context.Me.GetBatch(batch, o => o.Chats);
                context.Execute(batch);

                var primaryChat = me.Result.Chats.AsRequested().FirstOrDefault(i => i.ChatType == ChatTypeConstants.Group && i.Topic == GroupChatTopic);
                Assert.IsNotNull(primaryChat);

                batch = context.NewBatch();
                var chat = primaryChat.GetBatch(batch, o => o.Messages);
                context.Execute(batch);

                var chatMessages = chat.Result.Messages;
                Assert.IsNotNull(chatMessages);

                // Useful reference - https://docs.microsoft.com/en-us/graph/api/chatmessage-post?view=graph-rest-beta&tabs=http#example-5-sending-inline-images-along-with-the-message
                // assume as if there are no chat messages

                var body = $"<div><div><h1>Hello</h1><p>This is a unit test (AddChatMessageInlineImagesGroupBatchAsyncTest) posting a message with inline image</p><div><span><img height=\"392\" src=\"../hostedContents/1/$value\" width=\"300\" style=\"vertical-align:bottom; width:300px; height:392px\"></span></div></div></div>";

                ITeamChatMessageHostedContentCollection coll = new TeamChatMessageHostedContentCollection
                {

                };

                batch = context.NewBatch();
                chatMessages.AddBatch(batch, new ChatMessageOptions
                {
                    Content = body,
                    ContentType = ChatMessageContentType.Html,
                    HostedContents = {
                            new ChatMessageHostedContentOptions
                            {
                                Id = "1",
                                ContentBytes = "",
                                ContentType = "image/png"
                            }
                        }
                });

                context.Execute(batch);

                var batch2 = context.NewBatch();

                chat = chat.Result.GetBatch(batch2, o => o.Messages);
                context.Execute(batch2);
                var updateMessages = chat.Result.Messages.AsRequested();

                var message = updateMessages.Last();
                Assert.IsNotNull(message.CreatedDateTime);
                Assert.IsNotNull(message.Etag);
                Assert.IsNotNull(message.Importance);
                Assert.IsNotNull(message.LastModifiedDateTime);
                Assert.IsNotNull(message.Locale);
                Assert.IsNotNull(message.MessageType);
                Assert.IsNotNull(message.HostedContents);

                Assert.IsTrue(message.IsPropertyAvailable(o => o.ReplyToId));
                Assert.IsNull(message.ReplyToId);
                Assert.IsTrue(message.IsPropertyAvailable(o => o.Subject));
                Assert.IsNull(message.Subject);
                Assert.IsTrue(message.IsPropertyAvailable(o => o.Summary));
                Assert.IsNull(message.Summary);
            }
        }

        [TestMethod]
        public async Task AddChatMessageFileAttachmentOptionsGroupAsyncTest()
        {
            //TestCommon.Instance.Mocking = false;
            using (var context = await TestCommon.Instance.GetContextAsync(TestCommon.TestSite))
            {
                var me = context.Me.Get(o => o.Chats);
                Assert.IsTrue(me.Chats.Length > 0);

                var chat = me.Chats.AsRequested().FirstOrDefault(i => i.ChatType == ChatTypeConstants.Group && i.Topic == GroupChatTopic);
                Assert.IsNotNull(chat);

                chat.Load(o => o.Messages);
                var chatMessages = chat.Messages;

                Assert.IsNotNull(chatMessages);

                // Upload File to SharePoint Library - it will have to remain i guess as onetime upload.
                IFolder folder = await context.Web.Lists.GetByTitle("Documents").RootFolder.GetAsync();
                IFile existingFile = await folder.Files.FirstOrDefaultAsync(o => o.Name == "test_added.docx");
                if (existingFile == default)
                {
                    existingFile = await folder.Files.AddAsync("test_added.docx", System.IO.File.OpenRead($".{Path.DirectorySeparatorChar}TestAssets{Path.DirectorySeparatorChar}test.docx"));
                }

                Assert.IsNotNull(existingFile);
                Assert.AreEqual("test_added.docx", existingFile.Name);

                // Useful reference - https://docs.microsoft.com/en-us/graph/api/chatmessage-post?view=graph-rest-beta&tabs=http#example-4-file-attachments
                // assume as if there are no chat messages
                var attachmentId = existingFile.ETag.AsGraphEtag(); // Needs to be the documents eTag - just the GUID part
                var body = $"<h1>Hello</h1><br />This is a unit test with a file attachment (AddChatMessageFileAttachmentOptionsGroupAsyncTest) posting a message - <attachment id=\"{attachmentId}\"></attachment>";
                var subject = "Chat Message File Attachment Options Async Test";
                var fileUri = new Uri(existingFile.LinkingUrl);
                await context.ExecuteAsync();

                var batch = context.NewBatch();

                await chatMessages.AddBatchAsync(batch, new ChatMessageOptions
                {
                    Subject = subject,
                    Content = body,
                    ContentType = ChatMessageContentType.Html,
                    Attachments = {
                        new ChatMessageAttachmentOptions
                        {
                            Id = attachmentId,
                            ContentType = "reference",
                            // Cannot have the extension with a query graph doesnt recognise and think its part of file extension - include in docs.
                            ContentUrl = new Uri(fileUri.ToString().Replace(fileUri.Query, "")),
                            Name = $"{existingFile.Name}",
                            ThumbnailUrl = null,
                            Content = null
                        }
                    }
                });

                await context.ExecuteAsync(batch);

                chat = await chat.GetAsync(o => o.Messages);
                var updateMessages = chat.Messages;

                var message = updateMessages.AsEnumerable().OrderByDescending(o => o.Subject = subject).Last();
                Assert.IsNotNull(message.CreatedDateTime);

                Assert.IsNotNull(message.Etag);
                Assert.IsNotNull(message.Importance);
                Assert.IsNotNull(message.LastModifiedDateTime);
                Assert.IsNotNull(message.Locale);
                Assert.IsNotNull(message.MessageType);

                Assert.IsTrue(message.IsPropertyAvailable(o => o.ReplyToId));
                Assert.IsNull(message.ReplyToId);
                Assert.IsTrue(message.IsPropertyAvailable(o => o.Subject));
                Assert.IsNotNull(message.Subject);
                Assert.IsTrue(message.IsPropertyAvailable(o => o.Summary));
                Assert.IsNull(message.Summary);

                //delete file
                await existingFile.DeleteAsync(); //Note this will break the link in the Teams chat.

            }
        }

        [TestMethod]
        public async Task AddChatMessageSubjectGroupAsyncTest()
        {
            //TestCommon.Instance.Mocking = false;
            using (var context = await TestCommon.Instance.GetContextAsync(TestCommon.TestSite))
            {
                var me = context.Me.Get(o => o.Chats);
                Assert.IsTrue(me.Chats.Length > 0);

                var chat = me.Chats.AsRequested().FirstOrDefault(i => i.ChatType == ChatTypeConstants.Group && i.Topic == GroupChatTopic);
                Assert.IsNotNull(chat);

                chat.Load(o => o.Messages);
                var chatMessages = chat.Messages;

                Assert.IsNotNull(chatMessages);

                // assume as if there are no chat messages
                // There appears to be no remove option yet in this feature - so add a recognisable message
                var body = $"Hello, This is a unit test (AddChatMessageSubjectGroupAsyncTest) posting a message - PnP Rocks! - Woah...";
                await chatMessages.AddAsync(body, subject: "This is a subject test");

                chat = await chat.GetAsync(o => o.Messages);
                var updateMessages = chat.Messages.AsEnumerable();

                var message = updateMessages.Last();
                Assert.IsNotNull(message.CreatedDateTime);
                // Depending on regional settings this check might fail
                //Assert.AreEqual(message.DeletedDateTime, DateTime.MinValue);
                Assert.IsNotNull(message.Etag);
                Assert.IsNotNull(message.Importance);
                Assert.IsNotNull(message.LastModifiedDateTime);
                Assert.IsNotNull(message.Locale);
                Assert.IsNotNull(message.MessageType);

                Assert.IsTrue(message.IsPropertyAvailable(o => o.ReplyToId));
                Assert.IsNull(message.ReplyToId);
                // BERT - Check with PAUL
                //Assert.IsTrue(message.IsPropertyAvailable(o => o.Subject));
                //Assert.IsNotNull(message.Subject);
                Assert.IsTrue(message.IsPropertyAvailable(o => o.Summary));
                Assert.IsNull(message.Summary);

            }
        }

        [TestMethod]
        public async Task AddChatMessageThumbnailGroupAsyncTest()
        {
            //TestCommon.Instance.Mocking = false;
            using (var context = await TestCommon.Instance.GetContextAsync(TestCommon.TestSite))
            {
                var me = context.Me.Get(o => o.Chats);
                Assert.IsTrue(me.Chats.Length > 0);

                var chat = me.Chats.AsRequested().FirstOrDefault(i => i.ChatType == ChatTypeConstants.Group && i.Topic == GroupChatTopic);
                Assert.IsNotNull(chat);

                chat.Load(o => o.Messages);
                var chatMessages = chat.Messages;

                Assert.IsNotNull(chatMessages);

                // assume as if there are no chat messages
                // There appears to be no remove option yet in this feature - so add a recognisable message
                var attachmentId = "74d20c7f34aa4a7fb74e2b30004247c5";
                var body = $"<attachment id=\"{attachmentId}\"></attachment>";

                await chatMessages.AddAsync(new ChatMessageOptions
                {
                    Content = body,
                    ContentType = ChatMessageContentType.Html,
                    Attachments =
                        {
                             new ChatMessageAttachmentOptions
                            {
                                Id = attachmentId,
                                ContentType = "application/vnd.microsoft.card.thumbnail",
                                // Adaptive Card
                                Content = "{\r\n  \"title\": \"Unit Test posting a card\",\r\n  \"subtitle\": \"<h3>This is the subtitle</h3>\",\r\n  \"text\": \"Here is some body text. <br>\\r\\nAnd a <a href=\\\"http://microsoft.com/\\\">hyperlink</a>. <br>\\r\\nAnd below that is some buttons:\",\r\n  \"buttons\": [\r\n    {\r\n      \"type\": \"messageBack\",\r\n      \"title\": \"Login to FakeBot\",\r\n      \"text\": \"login\",\r\n      \"displayText\": \"login\",\r\n      \"value\": \"login\"\r\n    }\r\n  ]\r\n}",
                                ContentUrl = null,
                                Name = null,
                                ThumbnailUrl = null
                            }
                        }
                });

                chat = await chat.GetAsync(o => o.Messages);
                var updateMessages = chat.Messages.AsRequested();

                var message = updateMessages.Last();
                Assert.IsNotNull(message.CreatedDateTime);
                // Depending on regional settings this check might fail
                //Assert.AreEqual(message.DeletedDateTime, DateTime.MinValue);
                Assert.IsNotNull(message.Etag);
                Assert.IsNotNull(message.Importance);
                Assert.IsNotNull(message.LastModifiedDateTime);
                Assert.IsNotNull(message.Locale);
                Assert.IsNotNull(message.MessageType);

                Assert.IsTrue(message.IsPropertyAvailable(o => o.ReplyToId));
                Assert.IsNull(message.ReplyToId);
                Assert.IsTrue(message.IsPropertyAvailable(o => o.Subject));
                Assert.IsNull(message.Subject);
                Assert.IsTrue(message.IsPropertyAvailable(o => o.Summary));
                Assert.IsNull(message.Summary);
            }
        }

        [TestMethod]
        public async Task AddChatMessageAdaptiveGroupAsyncTest()
        {
            //Reference: https://docs.microsoft.com/en-us/microsoftteams/platform/task-modules-and-cards/cards/cards-reference#adaptive-card

            //TestCommon.Instance.Mocking = false;
            using (var context = await TestCommon.Instance.GetContextAsync(TestCommon.TestSite))
            {
                var me = context.Me.Get(o => o.Chats);
                Assert.IsTrue(me.Chats.Length > 0);

                var chat = me.Chats.AsRequested().FirstOrDefault(i => i.ChatType == ChatTypeConstants.Group && i.Topic == GroupChatTopic);
                Assert.IsNotNull(chat);

                chat.Load(o => o.Messages);
                var chatMessages = chat.Messages;

                Assert.IsNotNull(chatMessages);

                // assume as if there are no chat messages
                // There appears to be no remove option yet in this feature - so add a recognisable message
                var attachmentId = "74d20c7f34aa4a7fb74e2b30004247c5";
                var body = $"<attachment id=\"{attachmentId}\"></attachment>";

                await chatMessages.AddAsync(new ChatMessageOptions
                {
                    Content = body,
                    ContentType = ChatMessageContentType.Html,
                    Attachments = {
                        new ChatMessageAttachmentOptions
                        {
                            Id = attachmentId,
                            ContentType = "application/vnd.microsoft.card.adaptive",
                            // Adaptive Card
                            Content = "{\"$schema\":\"http://adaptivecards.io/schemas/adaptive-card.json\",\"type\":\"AdaptiveCard\",\"version\":\"1.0\",\"body\":[{\"type\":\"Container\",\"items\":[{\"type\":\"TextBlock\",\"text\":\"Adaptive Card Unit Test\",\"weight\":\"bolder\",\"size\":\"medium\"},{\"type\":\"ColumnSet\",\"columns\":[{\"type\":\"Column\",\"width\":\"auto\",\"items\":[{\"type\":\"Image\",\"url\":\"https://pbs.twimg.com/profile_images/3647943215/d7f12830b3c17a5a9e4afcc370e3a37e_400x400.jpeg\",\"size\":\"small\",\"style\":\"person\"}]},{\"type\":\"Column\",\"width\":\"stretch\",\"items\":[{\"type\":\"TextBlock\",\"text\":\"Matt Hidinger\",\"weight\":\"bolder\",\"wrap\":true},{\"type\":\"TextBlock\",\"spacing\":\"none\",\"text\":\"Created {{DATE(2017-02-14T06:08:39Z,SHORT)}}\",\"isSubtle\":true,\"wrap\":true}]}]}]},{\"type\":\"Container\",\"items\":[{\"type\":\"TextBlock\",\"text\":\"Now that we have defined the main rule sand features of the format ,we need to produce a schema and publish it to GitHub.The schema will be the starting point of our reference documentation.\",\"wrap\":true},{\"type\":\"FactSet\",\"facts\":[{\"title\":\"Board:\",\"value\":\"Adaptive Card\"},{\"title\":\"List:\",\"value\":\"Backlog\"},{\"title\":\"Assigned to:\",\"value\":\"Matt Hidinger\"},{\"title\":\"Duedate:\",\"value\":\"Not set\"}]}]}],\"actions\":[{\"type\":\"Action.ShowCard\",\"title\":\"Set due date\",\"card\":{\"type\":\"AdaptiveCard\",\"body\":[{\"type\":\"Input.Date\",\"id\":\"dueDate\"}],\"actions\":[{\"type\":\"Action.Submit\",\"title\":\"OK\"}]}},{\"type\":\"Action.ShowCard\",\"title\":\"Comment\",\"card\":{\"type\":\"AdaptiveCard\",\"body\":[{\"type\":\"Input.Text\",\"id\":\"comment\",\"isMultiline\":true,\"placeholder\":\"Enter your comment\"}],\"actions\":[{\"type\":\"Action.Submit\",\"title\":\"OK\"}]}}]}",
                            ContentUrl = null,
                            Name = null,
                            ThumbnailUrl = null
                        }
                    }
                });

                chat = await chat.GetAsync(o => o.Messages);
                var updateMessages = chat.Messages.AsEnumerable();

                var message = updateMessages.Last();
                Assert.IsNotNull(message.CreatedDateTime);
                Assert.IsNotNull(message.Etag);
                Assert.IsNotNull(message.Importance);
                Assert.IsNotNull(message.LastModifiedDateTime);
                Assert.IsNotNull(message.Locale);
                Assert.IsNotNull(message.MessageType);

                Assert.IsTrue(message.IsPropertyAvailable(o => o.ReplyToId));
                Assert.IsNull(message.ReplyToId);
                Assert.IsTrue(message.IsPropertyAvailable(o => o.Subject));
                Assert.IsNull(message.Subject);
                Assert.IsTrue(message.IsPropertyAvailable(o => o.Summary));
                Assert.IsNull(message.Summary);
            }
        }

        [TestMethod]
        public void AddChatMessageGroupBatchTest()
        {
            //TestCommon.Instance.Mocking = false;
            using (var context = TestCommon.Instance.GetContext(TestCommon.TestSite))
            {
                var batch = context.NewBatch();

                var me = context.Me.GetBatch(batch, o => o.Chats);
                context.Execute(batch);

                var primaryChat = me.Result.Chats.AsRequested().FirstOrDefault(i => i.ChatType == ChatTypeConstants.Group && i.Topic == GroupChatTopic);
                Assert.IsNotNull(primaryChat);

                batch = context.NewBatch();
                var chat = primaryChat.GetBatch(batch, o => o.Messages);
                context.Execute(batch);

                var chatMessages = chat.Result.Messages;
                Assert.IsNotNull(chatMessages);

                // assume as if there are no chat messages
                // There appears to be no remove option yet in this feature - so add a recognisable message
                var body = $"Hello, this is a unit test (AddChatMessageGroupBatchTest) posting a message - PnP Rocks! - Woah...";

                chatMessages.AddBatch(body, ChatMessageContentType.Text, "Batch Test");
                context.Execute();

                chat = primaryChat.GetBatch(o => o.Messages);
                context.Execute();
                var updateMessages = chat.Result.Messages;

                var message = updateMessages.AsRequested().FirstOrDefault(o => o.Body.Content == body);

                Assert.IsFalse(message == default);
                Assert.IsNotNull(message.CreatedDateTime);
                Assert.IsNotNull(message.Etag);
                Assert.IsNotNull(message.Importance);
                Assert.IsNotNull(message.LastModifiedDateTime);
                Assert.IsNotNull(message.Locale);
                Assert.IsNotNull(message.MessageType);

                Assert.IsTrue(message.IsPropertyAvailable(o => o.ReplyToId));
                Assert.IsNull(message.ReplyToId);
                Assert.IsTrue(message.IsPropertyAvailable(o => o.Subject));
                Assert.IsNotNull(message.Subject);
                Assert.IsTrue(message.IsPropertyAvailable(o => o.Summary));
                Assert.IsNull(message.Summary);

            }
        }

        [TestMethod]
        public async Task AddChatMessageBatchGroupAsyncTest()
        {
            //TestCommon.Instance.Mocking = false;
            using (var context = TestCommon.Instance.GetContext(TestCommon.TestSite))
            {
                var me = await context.Me.GetBatchAsync(o => o.Chats);
                await context.ExecuteAsync();
                Assert.IsTrue(me.Result.Chats.Length > 0);

                var chatQuery = me.Result.Chats.FirstOrDefault(i => i.ChatType == ChatTypeConstants.Group && i.Topic == GroupChatTopic);
                Assert.IsNotNull(chatQuery);

                var chat = await chatQuery.GetBatchAsync(o => o.Messages);
                await context.ExecuteAsync();
                var chatMessages = chat.Result.Messages;

                Assert.IsNotNull(chatMessages);

                // assume as if there are no chat messages
                // There appears to be no remove option yet in this feature - so add a recognisable message
                var body = $"Hello, this is a unit test (AddChatMessageBatchGroupAsyncTest) posting a message - PnP Rocks! - Woah...";
                await chatMessages.AddBatchAsync(body, ChatMessageContentType.Text, "Batch Test");
                await context.ExecuteAsync();

                chat = chatQuery.GetBatch(o => o.Messages);
                await context.ExecuteAsync();
                var updateMessages = chat.Result.Messages;

                var message = updateMessages.AsRequested().FirstOrDefault(o => o.Body.Content == body);

                Assert.IsFalse(message == default);
                Assert.IsNotNull(message.CreatedDateTime);
                // Depending on regional settings this check might fail
                //Assert.AreEqual(message.DeletedDateTime, DateTime.MinValue);
                Assert.IsNotNull(message.Etag);
                Assert.IsNotNull(message.Importance);
                Assert.IsNotNull(message.LastModifiedDateTime);
                Assert.IsNotNull(message.Locale);
                Assert.IsNotNull(message.MessageType);

                Assert.IsTrue(message.IsPropertyAvailable(o => o.ReplyToId));
                Assert.IsNull(message.ReplyToId);
                Assert.IsTrue(message.IsPropertyAvailable(o => o.Subject));
                Assert.IsNotNull(message.Subject);
                Assert.IsTrue(message.IsPropertyAvailable(o => o.Summary));
                Assert.IsNull(message.Summary);

            }
        }

        [TestMethod]
        public void AddChatMessageBatchOptionsGroupTest()
        {
            //TestCommon.Instance.Mocking = false;
            using (var context = TestCommon.Instance.GetContext(TestCommon.TestSite))
            {
                var me = context.Me.GetBatch(o => o.Chats);
                context.Execute();
                Assert.IsTrue(me.Result.Chats.Length > 0);

                var chatQuery = me.Result.Chats.FirstOrDefault(i => i.ChatType == ChatTypeConstants.Group && i.Topic == GroupChatTopic);
                Assert.IsNotNull(chatQuery);

                var chat = chatQuery.GetBatch(o => o.Messages);
                context.Execute();
                var chatMessages = chat.Result.Messages;

                Assert.IsNotNull(chatMessages);

                // assume as if there are no chat messages
                // There appears to be no remove option yet in this feature - so add a recognisable message
                var body = $"Hello, this is a unit test (AddChatMessageBatchOptionsGroupTest) posting a message - PnP Rocks! - Woah...";

                chatMessages.AddBatch(new ChatMessageOptions()
                {
                    Content = body,
                    Subject = "ChatMessageBatchOptionsTest"
                });
                context.Execute();


                chat = chatQuery.GetBatch(o => o.Messages);
                context.Execute();
                var updateMessages = chat.Result.Messages;

                var message = updateMessages.AsRequested().FirstOrDefault(o => o.Body.Content == body);

                Assert.IsFalse(message == default);
                Assert.IsNotNull(message.CreatedDateTime);
                // Depending on regional settings this check might fail
                //Assert.AreEqual(message.DeletedDateTime, DateTime.MinValue);
                Assert.IsNotNull(message.Etag);
                Assert.IsNotNull(message.Importance);
                Assert.IsNotNull(message.LastModifiedDateTime);
                Assert.IsNotNull(message.Locale);
                Assert.IsNotNull(message.MessageType);

                Assert.IsTrue(message.IsPropertyAvailable(o => o.ReplyToId));
                Assert.IsNull(message.ReplyToId);
                Assert.IsTrue(message.IsPropertyAvailable(o => o.Subject));
                Assert.IsNotNull(message.Subject);
                Assert.IsTrue(message.IsPropertyAvailable(o => o.Summary));
                Assert.IsNull(message.Summary);

            }
        }

        [TestMethod]
        public async Task AddChatMessageBatchOptionsGroupAsyncTest()
        {
            //TestCommon.Instance.Mocking = false;
            using (var context = TestCommon.Instance.GetContext(TestCommon.TestSite))
            {
                var me = await context.Me.GetBatchAsync(o => o.Chats);
                await context.ExecuteAsync();
                Assert.IsTrue(me.Result.Chats.Length > 0);

                var chatQuery = me.Result.Chats.FirstOrDefault(i => i.ChatType == ChatTypeConstants.Group && i.Topic == GroupChatTopic);
                Assert.IsNotNull(chatQuery);

                var chat = await chatQuery.GetBatchAsync(o => o.Messages);
                await context.ExecuteAsync();
                var chatMessages = chat.Result.Messages;

                Assert.IsNotNull(chatMessages);

                // assume as if there are no chat messages
                // There appears to be no remove option yet in this feature - so add a recognisable message
                var body = $"Hello, this is a unit test (AddChatMessageBatchOptionsGroupAsyncTest) posting a message - PnP Rocks! - Woah...";
                await chatMessages.AddBatchAsync(new ChatMessageOptions()
                {
                    Content = body,
                    Subject = "AddChatMessageBatchOptionsAsyncTest"
                });
                await context.ExecuteAsync();

                chat = chatQuery.GetBatch(o => o.Messages);
                await context.ExecuteAsync();
                var updateMessages = chat.Result.Messages;

                var message = updateMessages.AsRequested().FirstOrDefault(o => o.Body.Content == body);

                Assert.IsFalse(message == default);
                Assert.IsNotNull(message.CreatedDateTime);
                // Depending on regional settings this check might fail
                //Assert.AreEqual(message.DeletedDateTime, DateTime.MinValue);
                Assert.IsNotNull(message.Etag);
                Assert.IsNotNull(message.Importance);
                Assert.IsNotNull(message.LastModifiedDateTime);
                Assert.IsNotNull(message.Locale);
                Assert.IsNotNull(message.MessageType);

                Assert.IsTrue(message.IsPropertyAvailable(o => o.ReplyToId));
                Assert.IsNull(message.ReplyToId);
                Assert.IsTrue(message.IsPropertyAvailable(o => o.Subject));
                Assert.IsNotNull(message.Subject);
                Assert.IsTrue(message.IsPropertyAvailable(o => o.Summary));
                Assert.IsNull(message.Summary);

            }
        }

        [TestMethod]
        public void AddChatMessageSpecificGroupBatchTest()
        {
            //TestCommon.Instance.Mocking = false;
            using (var context = TestCommon.Instance.GetContext(TestCommon.TestSite))
            {
                var batch = context.NewBatch();
                var me = context.Me.GetBatch(batch, o => o.Chats);
                context.Execute(batch);
                Assert.IsTrue(me.Result.Chats.Length > 0);

                var chatQuery = me.Result.Chats.FirstOrDefault(i => i.ChatType == ChatTypeConstants.Group && i.Topic == GroupChatTopic);
                Assert.IsNotNull(chatQuery);

                var chat = chatQuery.GetBatch(batch, o => o.Messages);
                context.Execute(batch);
                var chatMessages = chat.Result.Messages;

                Assert.IsNotNull(chatMessages);

                // assume as if there are no chat messages
                // There appears to be no remove option yet in this feature - so add a recognisable message
                var body = $"Hello, this is a unit test (AddChatMessageSpecificGroupBatchTest) posting a message - PnP Rocks! - Woah...";
                chatMessages.AddBatch(batch, body);
                context.Execute(batch);

                var batch2 = context.NewBatch();
                chat = chatQuery.GetBatch(batch2, o => o.Messages);
                context.Execute(batch2);
                var updateMessages = chat.Result.Messages.AsRequested();

                var message = updateMessages.First(o => o.Body.Content == body);
                Assert.IsNotNull(message.CreatedDateTime);

                // Depending on regional settings this check might fail
                //Assert.AreEqual(message.DeletedDateTime, DateTime.MinValue);
                Assert.IsNotNull(message.Etag);
                Assert.IsNotNull(message.Importance);
                Assert.IsNotNull(message.LastModifiedDateTime);
                Assert.IsNotNull(message.Locale);
                Assert.IsNotNull(message.MessageType);

                Assert.IsTrue(message.IsPropertyAvailable(o => o.ReplyToId));
                Assert.IsNull(message.ReplyToId);
                Assert.IsTrue(message.IsPropertyAvailable(o => o.Subject));
                Assert.IsNull(message.Subject);
                Assert.IsTrue(message.IsPropertyAvailable(o => o.Summary));
                Assert.IsNull(message.Summary);
            }
        }

        [TestMethod]
        public async Task AddChatMessageSpecificBatchGroupAsyncTest()
        {
            //TestCommon.Instance.Mocking = false;
            using (var context = TestCommon.Instance.GetContext(TestCommon.TestSite))
            {
                var batch = context.NewBatch();
                var me = await context.Me.GetBatchAsync(batch, o => o.Chats);
                await context.ExecuteAsync(batch);
                Assert.IsTrue(me.Result.Chats.Length > 0);

                var chatQuery = me.Result.Chats.FirstOrDefault(i => i.ChatType == ChatTypeConstants.Group && i.Topic == GroupChatTopic);
                Assert.IsNotNull(chatQuery);

                var chat = await chatQuery.GetBatchAsync(batch, o => o.Messages);
                await context.ExecuteAsync(batch);
                var chatMessages = chat.Result.Messages;

                Assert.IsNotNull(chatMessages);

                // assume as if there are no chat messages
                // There appears to be no remove option yet in this feature - so add a recognisable message
                var body = $"Hello, this is a unit test (AddChatMessageSpecificBatchGroupAsyncTest) posting a message - PnP Rocks! - Woah...";
                await chatMessages.AddBatchAsync(batch, body, ChatMessageContentType.Text, "Batch Async Test");
                await context.ExecuteAsync(batch);

                var batch2 = context.NewBatch();
                chat = await chatQuery.GetBatchAsync(batch2, o => o.Messages);
                await context.ExecuteAsync(batch2);
                var updateMessages = chat.Result.Messages.AsRequested();

                var message = updateMessages.First(o => o.Body.Content == body);
                Assert.IsNotNull(message.CreatedDateTime);

                // Depending on regional settings this check might fail
                //Assert.AreEqual(message.DeletedDateTime, DateTime.MinValue);
                Assert.IsNotNull(message.Etag);
                Assert.IsNotNull(message.Importance);
                Assert.IsNotNull(message.LastModifiedDateTime);
                Assert.IsNotNull(message.Locale);
                Assert.IsNotNull(message.MessageType);

                Assert.IsTrue(message.IsPropertyAvailable(o => o.ReplyToId));
                Assert.IsNull(message.ReplyToId);
                Assert.IsTrue(message.IsPropertyAvailable(o => o.Subject));
                Assert.IsNotNull(message.Subject);
                Assert.IsTrue(message.IsPropertyAvailable(o => o.Summary));
                Assert.IsNull(message.Summary);
            }
        }

        [TestMethod]
        [ExpectedException(typeof(ArgumentNullException))]
        public void AddChatMessageBatchOptionsGroupExceptionTest()
        {
            //TestCommon.Instance.Mocking = false;
            using (var context = TestCommon.Instance.GetContext(TestCommon.TestSite))
            {
                var batch = context.NewBatch();
                var me = context.Me.GetBatch(batch, o => o.Chats);
                context.Execute(batch);
                Assert.IsTrue(me.Result.Chats.Length > 0);

                var chatQuery = me.Result.Chats.FirstOrDefault(i => i.ChatType == ChatTypeConstants.Group && i.Topic == GroupChatTopic);
                Assert.IsNotNull(chatQuery);

                var chat = chatQuery.GetBatch(batch, o => o.Messages);
                context.Execute(batch);
                var chatMessages = chat.Result.Messages;

                Assert.IsNotNull(chatMessages);

                // assume as if there are no chat messages
                // There appears to be no remove option yet in this feature - so add a recognisable message
                var body = string.Empty;
                chatMessages.AddBatch(batch, options: null);
                context.Execute(batch);

            }
        }

        [TestMethod]
        [ExpectedException(typeof(ArgumentNullException))]
        public void AddChatMessageBatchGroupExceptionTest()
        {
            //TestCommon.Instance.Mocking = false;
            using (var context = TestCommon.Instance.GetContext(TestCommon.TestSite))
            {
                var batch = context.NewBatch();
                var me = context.Me.GetBatch(batch, o => o.Chats);
                context.Execute(batch);
                Assert.IsTrue(me.Result.Chats.Length > 0);

                var chatQuery = me.Result.Chats.FirstOrDefault(i => i.ChatType == ChatTypeConstants.Group && i.Topic == GroupChatTopic);
                Assert.IsNotNull(chatQuery);

                var chat = chatQuery.GetBatch(batch, o => o.Messages);
                context.Execute(batch);
                var chatMessages = chat.Result.Messages;

                Assert.IsNotNull(chatMessages);

                // assume as if there are no chat messages
                // There appears to be no remove option yet in this feature - so add a recognisable message
                var body = string.Empty;
                chatMessages.AddBatch(batch, body);
                context.Execute(batch);

            }
        }

        [TestMethod]
        [ExpectedException(typeof(ArgumentNullException))]
        public void AddChatMessageGroupExceptionTest()
        {
            //TestCommon.Instance.Mocking = false;
            using (var context = TestCommon.Instance.GetContext(TestCommon.TestSite))
            {
                var me = context.Me.Get(o => o.Chats);
                Assert.IsTrue(me.Chats.Length > 0);

                var chat = me.Chats.AsRequested().FirstOrDefault(i => i.ChatType == ChatTypeConstants.Group && i.Topic == GroupChatTopic);
                Assert.IsNotNull(chat);

                chat.Load(o => o.Messages);
                var chatMessages = chat.Messages;

                Assert.IsNotNull(chatMessages);

                // assume as if there are no chat messages
                // There appears to be no remove option yet in this feature - so add a recognisable message
                var body = string.Empty;
                chatMessages.Add(body);
            }
        }

        [TestMethod]
        public async Task GetChatMessageMeetingAsyncTest()
        {
            //TestCommon.Instance.Mocking = false;
            using (var context = await TestCommon.Instance.GetContextAsync(TestCommon.TestSite))
            {
                var me = await context.Me.GetAsync(o => o.Chats);
                Assert.IsTrue(me.Chats.Length > 0);

                var chat = me.Chats.AsRequested().FirstOrDefault(i => i.ChatType == ChatTypeConstants.Meeting);
                Assert.IsNotNull(chat);

                chat = await chat.GetAsync(o => o.Messages);
                var chatMessages = chat.Messages;

                Assert.IsNotNull(chatMessages);

                // assume as if there are no chat messages
                // There appears to be no remove option yet in this feature - so add a recognisable message
                var body = "Hello, this is a unit test (GetChatMessageMeetingAsyncTest) posting a message - PnP Rocks!";
                await chatMessages.AddAsync(body);

                chat = await chat.GetAsync(o => o.Messages);
                var updateMessages = chat.Messages.AsRequested();

                var message = updateMessages.First(o => o.Body.Content == body);
                Assert.IsNotNull(message.CreatedDateTime);
                Assert.IsNotNull(message.Etag);
                Assert.IsNotNull(message.Importance);
                Assert.IsNotNull(message.LastModifiedDateTime);
                Assert.IsNotNull(message.Locale);
                Assert.IsNotNull(message.MessageType);

                Assert.IsTrue(message.IsPropertyAvailable(o => o.ReplyToId));
                Assert.IsNull(message.ReplyToId);
                Assert.IsTrue(message.IsPropertyAvailable(o => o.Subject));
                Assert.IsNull(message.Subject);
                Assert.IsTrue(message.IsPropertyAvailable(o => o.Summary));
                Assert.IsNull(message.Summary);
            }
        }


        [TestMethod]
        public void AddChatMessageToMeetingChatTest()
        {
            //TestCommon.Instance.Mocking = false;
            using (var context = TestCommon.Instance.GetContext(TestCommon.TestSite))
            {
                var me = context.Me.Get(o => o.Chats);
                Assert.IsTrue(me.Chats.Length > 0);

                var chat = me.Chats.AsRequested().FirstOrDefault(i => i.ChatType == ChatTypeConstants.Meeting);
                Assert.IsNotNull(chat);

                chat.Load(o => o.Messages);
                var chatMessages = chat.Messages;

                Assert.IsNotNull(chatMessages);

                // assume as if there are no chat messages
                // There appears to be no remove option yet in this feature - so add a recognisable message
                var body = $"Hello, this is a unit test (AddChatMessageToMeetingChatTest) posting a message - PnP Rocks! - Woah...";
                chatMessages.Add(body);

                chat = chat.Get(o => o.Messages);
                var updateMessages = chat.Messages.AsRequested();

                var message = updateMessages.First(o => o.Body.Content == body);
                Assert.IsNotNull(message.CreatedDateTime);
                Assert.IsNotNull(message.Etag);
                Assert.IsNotNull(message.Importance);
                Assert.IsNotNull(message.LastModifiedDateTime);
                Assert.IsNotNull(message.Locale);
                Assert.IsNotNull(message.MessageType);

                Assert.IsTrue(message.IsPropertyAvailable(o => o.ReplyToId));
                Assert.IsNull(message.ReplyToId);
                Assert.IsTrue(message.IsPropertyAvailable(o => o.Subject));
                Assert.IsNull(message.Subject);
                Assert.IsTrue(message.IsPropertyAvailable(o => o.Summary));
                Assert.IsNull(message.Summary);
            }
        }

        [TestMethod]
        public void AddChatMessageOptionsMeetingTest()
        {
            //TestCommon.Instance.Mocking = false;
            using (var context = TestCommon.Instance.GetContext(TestCommon.TestSite))
            {
                var me = context.Me.Get(o => o.Chats);
                Assert.IsTrue(me.Chats.Length > 0);

                var chat = me.Chats.AsRequested().FirstOrDefault(i => i.ChatType == ChatTypeConstants.Meeting);
                Assert.IsNotNull(chat);

                chat.Load(o => o.Messages);
                var chatMessages = chat.Messages;

                Assert.IsNotNull(chatMessages);

                // assume as if there are no chat messages
                // There appears to be no remove option yet in this feature - so add a recognisable message
                var body = $"Hello, this is a unit test (AddChatMessageOptionsMeetingTest) posting a message - PnP Rocks! - Woah...";
                var subject = "Options";
                chatMessages.Add(new ChatMessageOptions()
                {
                    Content = body,
                    Subject = subject
                });

                chat = chat.Get(o => o.Messages);
                var updateMessages = chat.Messages.AsRequested();

                var message = updateMessages.First(o => o.Body.Content == body);
                Assert.IsNotNull(message.CreatedDateTime);
                Assert.IsNotNull(message.Etag);
                Assert.IsNotNull(message.Importance);
                Assert.IsNotNull(message.LastModifiedDateTime);
                Assert.IsNotNull(message.Locale);
                Assert.IsNotNull(message.MessageType);

                Assert.IsTrue(message.IsPropertyAvailable(o => o.ReplyToId));
                Assert.IsNull(message.ReplyToId);
                Assert.IsTrue(message.IsPropertyAvailable(o => o.Subject));
                Assert.IsNotNull(message.Subject);
                Assert.IsTrue(message.IsPropertyAvailable(o => o.Summary));
                Assert.IsNull(message.Summary);
            }
        }

        [TestMethod]
        public async Task AddChatMessageHtmlMeetingAsyncTest()
        {
            //TestCommon.Instance.Mocking = false;
            using (var context = await TestCommon.Instance.GetContextAsync(TestCommon.TestSite))
            {
                var me = context.Me.Get(o => o.Chats);
                Assert.IsTrue(me.Chats.Length > 0);

                var chat = me.Chats.AsRequested().FirstOrDefault(i => i.ChatType == ChatTypeConstants.Meeting);
                Assert.IsNotNull(chat);

                chat.Load(o => o.Messages);
                var chatMessages = chat.Messages;

                Assert.IsNotNull(chatMessages);

                // assume as if there are no chat messages
                // There appears to be no remove option yet in this feature - so add a recognisable message
                var body = $"<h1>Hello</h1><br />This is a unit test (AddChatMessageHtmlMeetingAsyncTest) posting a message - <strong>PnP Rocks!</strong> - Woah...";

                await chatMessages.AddAsync(body, ChatMessageContentType.Html);

                chat = await chat.GetAsync(o => o.Messages);
                var updateMessages = chat.Messages;

                var message = updateMessages.AsEnumerable().Last();
                Assert.IsNotNull(message.CreatedDateTime);
                // Depending on regional settings this check might fail
                //Assert.AreEqual(message.DeletedDateTime, DateTime.MinValue);
                Assert.IsNotNull(message.Etag);
                Assert.IsNotNull(message.Importance);
                Assert.IsNotNull(message.LastModifiedDateTime);
                Assert.IsNotNull(message.Locale);
                Assert.IsNotNull(message.MessageType);

                Assert.IsTrue(message.IsPropertyAvailable(o => o.ReplyToId));
                Assert.IsNull(message.ReplyToId);
                Assert.IsTrue(message.IsPropertyAvailable(o => o.Subject));
                Assert.IsNull(message.Subject);
                Assert.IsTrue(message.IsPropertyAvailable(o => o.Summary));
                Assert.IsNull(message.Summary);

            }
        }

        [TestMethod]
        public async Task AddChatMessageFileAttachmentMeetingAsyncTest()
        {
            //TestCommon.Instance.Mocking = false;
            using (var context = await TestCommon.Instance.GetContextAsync(TestCommon.TestSite))
            {
                var me = context.Me.Get(o => o.Chats);
                Assert.IsTrue(me.Chats.Length > 0);

                var chat = me.Chats.AsRequested().FirstOrDefault(i => i.ChatType == ChatTypeConstants.Meeting);
                Assert.IsNotNull(chat);

                chat.Load(o => o.Messages);
                var chatMessages = chat.Messages;

                Assert.IsNotNull(chatMessages);

                // Upload File to SharePoint Library - it will have to remain i guess as onetime upload.
                IFolder folder = await context.Web.Lists.GetByTitle("Documents").RootFolder.GetAsync();
                IFile existingFile = await folder.Files.FirstOrDefaultAsync(o => o.Name == "test_added.docx");
                if (existingFile == default)
                {
                    existingFile = await folder.Files.AddAsync("test_added.docx", System.IO.File.OpenRead($".{Path.DirectorySeparatorChar}TestAssets{Path.DirectorySeparatorChar}test.docx"));
                }

                Assert.IsNotNull(existingFile);
                Assert.AreEqual("test_added.docx", existingFile.Name);

                // Useful reference - https://docs.microsoft.com/en-us/graph/api/chatmessage-post?view=graph-rest-beta&tabs=http#example-4-file-attachments
                // assume as if there are no chat messages
                var attachmentId = existingFile.ETag.AsGraphEtag(); // Needs to be the documents eTag - just the GUID part
                var body = $"<h1>Hello</h1><br />This is a unit test with a file attachment (AddChatMessageFileAttachmentMeetingAsyncTest) posting a message - <attachment id=\"{attachmentId}\"></attachment>";

                var fileUri = new Uri(existingFile.LinkingUrl);

                await chatMessages.AddAsync(new ChatMessageOptions
                {
                    Content = body,
                    ContentType = ChatMessageContentType.Html,
                    Attachments = {
                        new ChatMessageAttachmentOptions
                        {
                            Id = attachmentId,
                            ContentType = "reference",
                            // Cannot have the extension with a query graph doesnt recognise and think its part of file extension - include in docs.
                            ContentUrl = new Uri(fileUri.ToString().Replace(fileUri.Query, "")),
                            Name = $"{existingFile.Name}",
                            ThumbnailUrl = null,
                            Content = null
                        }
                    }
                });

                chat = await chat.GetAsync(o => o.Messages);
                var updateMessages = chat.Messages;

                var message = updateMessages.AsEnumerable().Last();
                Assert.IsNotNull(message.CreatedDateTime);
                Assert.IsNotNull(message.Etag);
                Assert.IsNotNull(message.Importance);
                Assert.IsNotNull(message.LastModifiedDateTime);
                Assert.IsNotNull(message.Locale);
                Assert.IsNotNull(message.MessageType);

                Assert.IsTrue(message.IsPropertyAvailable(o => o.ReplyToId));
                Assert.IsNull(message.ReplyToId);
                Assert.IsTrue(message.IsPropertyAvailable(o => o.Subject));
                Assert.IsNull(message.Subject);
                Assert.IsTrue(message.IsPropertyAvailable(o => o.Summary));
                Assert.IsNull(message.Summary);

                //delete file
                await existingFile.DeleteAsync(); //Note this will break the link in the Teams chat.
            }
        }

        [TestMethod]
        public async Task AddChatMessageInlineImagesMeetingAsyncTest()
        {
            //TestCommon.Instance.Mocking = false;
            using (var context = await TestCommon.Instance.GetContextAsync(TestCommon.TestSite))
            {
                var me = context.Me.Get(o => o.Chats);
                Assert.IsTrue(me.Chats.Length > 0);

                var chat = me.Chats.AsRequested().FirstOrDefault(i => i.ChatType == ChatTypeConstants.Meeting);
                Assert.IsNotNull(chat);

                chat.Load(o => o.Messages);
                var chatMessages = chat.Messages;

                Assert.IsNotNull(chatMessages);

                // Useful reference - https://docs.microsoft.com/en-us/graph/api/chatmessage-post?view=graph-rest-beta&tabs=http#example-5-sending-inline-images-along-with-the-message
                // assume as if there are no chat messages

                var body = $"<div><div><h1>Hello</h1><p>This is a unit test (AddChatMessageInlineImagesMeetingAsyncTest) posting a message with inline image</p><div><span><img height=\"392\" src=\"../hostedContents/1/$value\" width=\"300\" style=\"vertical-align:bottom; width:300px; height:392px\"></span></div></div></div>";

                await chatMessages.AddAsync(new ChatMessageOptions
                {
                    Content = body,
                    ContentType = ChatMessageContentType.Html,
                    HostedContents =
                    {
                        new ChatMessageHostedContentOptions
                        {
                            Id = "1",
                            ContentBytes = "",
                            ContentType = "image/png"
                        }
                    }
                });

                chat = await chat.GetAsync(o => o.Messages);
                var updateMessages = chat.Messages;

                var message = updateMessages.AsEnumerable().Last();
                Assert.IsNotNull(message.CreatedDateTime);
                // Depending on regional settings this check might fail
                //Assert.AreEqual(message.DeletedDateTime, DateTime.MinValue);
                Assert.IsNotNull(message.Etag);
                Assert.IsNotNull(message.Importance);
                Assert.IsNotNull(message.LastModifiedDateTime);
                Assert.IsNotNull(message.Locale);
                Assert.IsNotNull(message.MessageType);
                Assert.IsNotNull(message.HostedContents);

                Assert.IsTrue(message.IsPropertyAvailable(o => o.ReplyToId));
                Assert.IsNull(message.ReplyToId);
                Assert.IsTrue(message.IsPropertyAvailable(o => o.Subject));
                Assert.IsNull(message.Subject);
                Assert.IsTrue(message.IsPropertyAvailable(o => o.Summary));
                Assert.IsNull(message.Summary);

            }
        }

        [TestMethod]
        public void AddChatMessageInlineImagesMeetingBatchAsyncTest()
        {
            //TestCommon.Instance.Mocking = false;
            using (var context = TestCommon.Instance.GetContext(TestCommon.TestSite))
            {
                var batch = context.NewBatch();

                var me = context.Me.GetBatch(batch, o => o.Chats);
                context.Execute(batch);

                var primaryChat = me.Result.Chats.AsRequested().FirstOrDefault(i => i.ChatType == ChatTypeConstants.Meeting);
                Assert.IsNotNull(primaryChat);

                batch = context.NewBatch();
                var chat = primaryChat.GetBatch(batch, o => o.Messages);
                context.Execute(batch);

                var chatMessages = chat.Result.Messages;
                Assert.IsNotNull(chatMessages);

                // Useful reference - https://docs.microsoft.com/en-us/graph/api/chatmessage-post?view=graph-rest-beta&tabs=http#example-5-sending-inline-images-along-with-the-message
                // assume as if there are no chat messages

                var body = $"<div><div><h1>Hello</h1><p>This is a unit test (AddChatMessageInlineImagesMeetingBatchAsyncTest) posting a message with inline image</p><div><span><img height=\"392\" src=\"../hostedContents/1/$value\" width=\"300\" style=\"vertical-align:bottom; width:300px; height:392px\"></span></div></div></div>";

                ITeamChatMessageHostedContentCollection coll = new TeamChatMessageHostedContentCollection
                {

                };

                batch = context.NewBatch();
                chatMessages.AddBatch(batch, new ChatMessageOptions
                {
                    Content = body,
                    ContentType = ChatMessageContentType.Html,
                    HostedContents = {
                            new ChatMessageHostedContentOptions
                            {
                                Id = "1",
                                ContentBytes = "",
                                ContentType = "image/png"
                            }
                        }
                });

                context.Execute(batch);

                var batch2 = context.NewBatch();

                chat = chat.Result.GetBatch(batch2, o => o.Messages);
                context.Execute(batch2);
                var updateMessages = chat.Result.Messages.AsRequested();

                var message = updateMessages.Last();
                Assert.IsNotNull(message.CreatedDateTime);
                Assert.IsNotNull(message.Etag);
                Assert.IsNotNull(message.Importance);
                Assert.IsNotNull(message.LastModifiedDateTime);
                Assert.IsNotNull(message.Locale);
                Assert.IsNotNull(message.MessageType);
                Assert.IsNotNull(message.HostedContents);

                Assert.IsTrue(message.IsPropertyAvailable(o => o.ReplyToId));
                Assert.IsNull(message.ReplyToId);
                Assert.IsTrue(message.IsPropertyAvailable(o => o.Subject));
                Assert.IsNull(message.Subject);
                Assert.IsTrue(message.IsPropertyAvailable(o => o.Summary));
                Assert.IsNull(message.Summary);
            }
        }

        [TestMethod]
        public async Task AddChatMessageFileAttachmentOptionsMeetingAsyncTest()
        {
            //TestCommon.Instance.Mocking = false;
            using (var context = await TestCommon.Instance.GetContextAsync(TestCommon.TestSite))
            {
                var me = context.Me.Get(o => o.Chats);
                Assert.IsTrue(me.Chats.Length > 0);

                var chat = me.Chats.AsRequested().FirstOrDefault(i => i.ChatType == ChatTypeConstants.Meeting);
                Assert.IsNotNull(chat);

                chat.Load(o => o.Messages);
                var chatMessages = chat.Messages;

                Assert.IsNotNull(chatMessages);

                // Upload File to SharePoint Library - it will have to remain i guess as onetime upload.
                IFolder folder = await context.Web.Lists.GetByTitle("Documents").RootFolder.GetAsync();
                IFile existingFile = await folder.Files.FirstOrDefaultAsync(o => o.Name == "test_added.docx");
                if (existingFile == default)
                {
                    existingFile = await folder.Files.AddAsync("test_added.docx", System.IO.File.OpenRead($".{Path.DirectorySeparatorChar}TestAssets{Path.DirectorySeparatorChar}test.docx"));
                }

                Assert.IsNotNull(existingFile);
                Assert.AreEqual("test_added.docx", existingFile.Name);

                // Useful reference - https://docs.microsoft.com/en-us/graph/api/chatmessage-post?view=graph-rest-beta&tabs=http#example-4-file-attachments
                // assume as if there are no chat messages
                var attachmentId = existingFile.ETag.AsGraphEtag(); // Needs to be the documents eTag - just the GUID part
                var body = $"<h1>Hello</h1><br />This is a unit test with a file attachment (AddChatMessageFileAttachmentOptionsMeetingAsyncTest) posting a message - <attachment id=\"{attachmentId}\"></attachment>";
                var subject = "Chat Message File Attachment Options Async Test";
                var fileUri = new Uri(existingFile.LinkingUrl);
                await context.ExecuteAsync();

                var batch = context.NewBatch();

                await chatMessages.AddBatchAsync(batch, new ChatMessageOptions
                {
                    Subject = subject,
                    Content = body,
                    ContentType = ChatMessageContentType.Html,
                    Attachments = {
                        new ChatMessageAttachmentOptions
                        {
                            Id = attachmentId,
                            ContentType = "reference",
                            // Cannot have the extension with a query graph doesnt recognise and think its part of file extension - include in docs.
                            ContentUrl = new Uri(fileUri.ToString().Replace(fileUri.Query, "")),
                            Name = $"{existingFile.Name}",
                            ThumbnailUrl = null,
                            Content = null
                        }
                    }
                });

                await context.ExecuteAsync(batch);

                chat = await chat.GetAsync(o => o.Messages);
                var updateMessages = chat.Messages;

                var message = updateMessages.AsEnumerable().OrderByDescending(o => o.Subject = subject).Last();
                Assert.IsNotNull(message.CreatedDateTime);

                Assert.IsNotNull(message.Etag);
                Assert.IsNotNull(message.Importance);
                Assert.IsNotNull(message.LastModifiedDateTime);
                Assert.IsNotNull(message.Locale);
                Assert.IsNotNull(message.MessageType);

                Assert.IsTrue(message.IsPropertyAvailable(o => o.ReplyToId));
                Assert.IsNull(message.ReplyToId);
                Assert.IsTrue(message.IsPropertyAvailable(o => o.Subject));
                Assert.IsNotNull(message.Subject);
                Assert.IsTrue(message.IsPropertyAvailable(o => o.Summary));
                Assert.IsNull(message.Summary);

                //delete file
                await existingFile.DeleteAsync(); //Note this will break the link in the Teams chat.

            }
        }

        [TestMethod]
        public async Task AddChatMessageSubjectMeetingAsyncTest()
        {
            //TestCommon.Instance.Mocking = false;
            using (var context = await TestCommon.Instance.GetContextAsync(TestCommon.TestSite))
            {
                var me = context.Me.Get(o => o.Chats);
                Assert.IsTrue(me.Chats.Length > 0);

                var chat = me.Chats.AsRequested().FirstOrDefault(i => i.ChatType == ChatTypeConstants.Meeting);
                Assert.IsNotNull(chat);

                chat.Load(o => o.Messages);
                var chatMessages = chat.Messages;

                Assert.IsNotNull(chatMessages);

                // assume as if there are no chat messages
                // There appears to be no remove option yet in this feature - so add a recognisable message
                var body = $"Hello, This is a unit test (AddChatMessageSubjectMeetingAsyncTest) posting a message - PnP Rocks! - Woah...";
                await chatMessages.AddAsync(body, subject: "This is a subject test");

                chat = await chat.GetAsync(o => o.Messages);
                var updateMessages = chat.Messages.AsEnumerable();

                var message = updateMessages.Last();
                Assert.IsNotNull(message.CreatedDateTime);
                // Depending on regional settings this check might fail
                //Assert.AreEqual(message.DeletedDateTime, DateTime.MinValue);
                Assert.IsNotNull(message.Etag);
                Assert.IsNotNull(message.Importance);
                Assert.IsNotNull(message.LastModifiedDateTime);
                Assert.IsNotNull(message.Locale);
                Assert.IsNotNull(message.MessageType);

                Assert.IsTrue(message.IsPropertyAvailable(o => o.ReplyToId));
                Assert.IsNull(message.ReplyToId);
                // BERT - Check with PAUL
                //Assert.IsTrue(message.IsPropertyAvailable(o => o.Subject));
                //Assert.IsNotNull(message.Subject);
                Assert.IsTrue(message.IsPropertyAvailable(o => o.Summary));
                Assert.IsNull(message.Summary);

            }
        }

        [TestMethod]
        public async Task AddChatMessageThumbnailMeetingAsyncTest()
        {
            //TestCommon.Instance.Mocking = false;
            using (var context = await TestCommon.Instance.GetContextAsync(TestCommon.TestSite))
            {
                var me = context.Me.Get(o => o.Chats);
                Assert.IsTrue(me.Chats.Length > 0);

                var chat = me.Chats.AsRequested().FirstOrDefault(i => i.ChatType == ChatTypeConstants.Meeting);
                Assert.IsNotNull(chat);

                chat.Load(o => o.Messages);
                var chatMessages = chat.Messages;

                Assert.IsNotNull(chatMessages);

                // assume as if there are no chat messages
                // There appears to be no remove option yet in this feature - so add a recognisable message
                var attachmentId = "74d20c7f34aa4a7fb74e2b30004247c5";
                var body = $"<attachment id=\"{attachmentId}\"></attachment>";

                await chatMessages.AddAsync(new ChatMessageOptions
                {
                    Content = body,
                    ContentType = ChatMessageContentType.Html,
                    Attachments =
                        {
                             new ChatMessageAttachmentOptions
                            {
                                Id = attachmentId,
                                ContentType = "application/vnd.microsoft.card.thumbnail",
                                // Adaptive Card
                                Content = "{\r\n  \"title\": \"Unit Test posting a card\",\r\n  \"subtitle\": \"<h3>This is the subtitle</h3>\",\r\n  \"text\": \"Here is some body text. <br>\\r\\nAnd a <a href=\\\"http://microsoft.com/\\\">hyperlink</a>. <br>\\r\\nAnd below that is some buttons:\",\r\n  \"buttons\": [\r\n    {\r\n      \"type\": \"messageBack\",\r\n      \"title\": \"Login to FakeBot\",\r\n      \"text\": \"login\",\r\n      \"displayText\": \"login\",\r\n      \"value\": \"login\"\r\n    }\r\n  ]\r\n}",
                                ContentUrl = null,
                                Name = null,
                                ThumbnailUrl = null
                            }
                        }
                });

                chat = await chat.GetAsync(o => o.Messages);
                var updateMessages = chat.Messages.AsRequested();

                var message = updateMessages.Last();
                Assert.IsNotNull(message.CreatedDateTime);
                // Depending on regional settings this check might fail
                //Assert.AreEqual(message.DeletedDateTime, DateTime.MinValue);
                Assert.IsNotNull(message.Etag);
                Assert.IsNotNull(message.Importance);
                Assert.IsNotNull(message.LastModifiedDateTime);
                Assert.IsNotNull(message.Locale);
                Assert.IsNotNull(message.MessageType);

                Assert.IsTrue(message.IsPropertyAvailable(o => o.ReplyToId));
                Assert.IsNull(message.ReplyToId);
                Assert.IsTrue(message.IsPropertyAvailable(o => o.Subject));
                Assert.IsNull(message.Subject);
                Assert.IsTrue(message.IsPropertyAvailable(o => o.Summary));
                Assert.IsNull(message.Summary);
            }
        }

        [TestMethod]
        public async Task AddChatMessageAdaptiveMeetingAsyncTest()
        {
            //Reference: https://docs.microsoft.com/en-us/microsoftteams/platform/task-modules-and-cards/cards/cards-reference#adaptive-card

            //TestCommon.Instance.Mocking = false;
            using (var context = await TestCommon.Instance.GetContextAsync(TestCommon.TestSite))
            {
                var me = context.Me.Get(o => o.Chats);
                Assert.IsTrue(me.Chats.Length > 0);

                var chat = me.Chats.AsRequested().FirstOrDefault(i => i.ChatType == ChatTypeConstants.Meeting);
                Assert.IsNotNull(chat);

                chat.Load(o => o.Messages);
                var chatMessages = chat.Messages;

                Assert.IsNotNull(chatMessages);

                // assume as if there are no chat messages
                // There appears to be no remove option yet in this feature - so add a recognisable message
                var attachmentId = "74d20c7f34aa4a7fb74e2b30004247c5";
                var body = $"<attachment id=\"{attachmentId}\"></attachment>";

                await chatMessages.AddAsync(new ChatMessageOptions
                {
                    Content = body,
                    ContentType = ChatMessageContentType.Html,
                    Attachments = {
                        new ChatMessageAttachmentOptions
                        {
                            Id = attachmentId,
                            ContentType = "application/vnd.microsoft.card.adaptive",
                            // Adaptive Card
                            Content = "{\"$schema\":\"http://adaptivecards.io/schemas/adaptive-card.json\",\"type\":\"AdaptiveCard\",\"version\":\"1.0\",\"body\":[{\"type\":\"Container\",\"items\":[{\"type\":\"TextBlock\",\"text\":\"Adaptive Card Unit Test\",\"weight\":\"bolder\",\"size\":\"medium\"},{\"type\":\"ColumnSet\",\"columns\":[{\"type\":\"Column\",\"width\":\"auto\",\"items\":[{\"type\":\"Image\",\"url\":\"https://pbs.twimg.com/profile_images/3647943215/d7f12830b3c17a5a9e4afcc370e3a37e_400x400.jpeg\",\"size\":\"small\",\"style\":\"person\"}]},{\"type\":\"Column\",\"width\":\"stretch\",\"items\":[{\"type\":\"TextBlock\",\"text\":\"Matt Hidinger\",\"weight\":\"bolder\",\"wrap\":true},{\"type\":\"TextBlock\",\"spacing\":\"none\",\"text\":\"Created {{DATE(2017-02-14T06:08:39Z,SHORT)}}\",\"isSubtle\":true,\"wrap\":true}]}]}]},{\"type\":\"Container\",\"items\":[{\"type\":\"TextBlock\",\"text\":\"Now that we have defined the main rule sand features of the format ,we need to produce a schema and publish it to GitHub.The schema will be the starting point of our reference documentation.\",\"wrap\":true},{\"type\":\"FactSet\",\"facts\":[{\"title\":\"Board:\",\"value\":\"Adaptive Card\"},{\"title\":\"List:\",\"value\":\"Backlog\"},{\"title\":\"Assigned to:\",\"value\":\"Matt Hidinger\"},{\"title\":\"Duedate:\",\"value\":\"Not set\"}]}]}],\"actions\":[{\"type\":\"Action.ShowCard\",\"title\":\"Set due date\",\"card\":{\"type\":\"AdaptiveCard\",\"body\":[{\"type\":\"Input.Date\",\"id\":\"dueDate\"}],\"actions\":[{\"type\":\"Action.Submit\",\"title\":\"OK\"}]}},{\"type\":\"Action.ShowCard\",\"title\":\"Comment\",\"card\":{\"type\":\"AdaptiveCard\",\"body\":[{\"type\":\"Input.Text\",\"id\":\"comment\",\"isMultiline\":true,\"placeholder\":\"Enter your comment\"}],\"actions\":[{\"type\":\"Action.Submit\",\"title\":\"OK\"}]}}]}",
                            ContentUrl = null,
                            Name = null,
                            ThumbnailUrl = null
                        }
                    }
                });

                chat = await chat.GetAsync(o => o.Messages);
                var updateMessages = chat.Messages.AsEnumerable();

                var message = updateMessages.Last();
                Assert.IsNotNull(message.CreatedDateTime);
                Assert.IsNotNull(message.Etag);
                Assert.IsNotNull(message.Importance);
                Assert.IsNotNull(message.LastModifiedDateTime);
                Assert.IsNotNull(message.Locale);
                Assert.IsNotNull(message.MessageType);

                Assert.IsTrue(message.IsPropertyAvailable(o => o.ReplyToId));
                Assert.IsNull(message.ReplyToId);
                Assert.IsTrue(message.IsPropertyAvailable(o => o.Subject));
                Assert.IsNull(message.Subject);
                Assert.IsTrue(message.IsPropertyAvailable(o => o.Summary));
                Assert.IsNull(message.Summary);
            }
        }

        [TestMethod]
        public void AddChatMessageMeetingBatchTest()
        {
            //TestCommon.Instance.Mocking = false;
            using (var context = TestCommon.Instance.GetContext(TestCommon.TestSite))
            {
                var batch = context.NewBatch();

                var me = context.Me.GetBatch(batch, o => o.Chats);
                context.Execute(batch);

                var primaryChat = me.Result.Chats.AsRequested().FirstOrDefault(i => i.ChatType == ChatTypeConstants.Meeting);
                Assert.IsNotNull(primaryChat);

                batch = context.NewBatch();
                var chat = primaryChat.GetBatch(batch, o => o.Messages);
                context.Execute(batch);

                var chatMessages = chat.Result.Messages;
                Assert.IsNotNull(chatMessages);

                // assume as if there are no chat messages
                // There appears to be no remove option yet in this feature - so add a recognisable message
                var body = $"Hello, this is a unit test (AddChatMessageMeetingBatchTest) posting a message - PnP Rocks! - Woah...";

                chatMessages.AddBatch(body, ChatMessageContentType.Text, "Batch Test");
                context.Execute();

                chat = primaryChat.GetBatch(o => o.Messages);
                context.Execute();
                var updateMessages = chat.Result.Messages;

                var message = updateMessages.AsRequested().FirstOrDefault(o => o.Body.Content == body);

                Assert.IsFalse(message == default);
                Assert.IsNotNull(message.CreatedDateTime);
                Assert.IsNotNull(message.Etag);
                Assert.IsNotNull(message.Importance);
                Assert.IsNotNull(message.LastModifiedDateTime);
                Assert.IsNotNull(message.Locale);
                Assert.IsNotNull(message.MessageType);

                Assert.IsTrue(message.IsPropertyAvailable(o => o.ReplyToId));
                Assert.IsNull(message.ReplyToId);
                Assert.IsTrue(message.IsPropertyAvailable(o => o.Subject));
                Assert.IsNotNull(message.Subject);
                Assert.IsTrue(message.IsPropertyAvailable(o => o.Summary));
                Assert.IsNull(message.Summary);

            }
        }

        [TestMethod]
        public async Task AddChatMessageBatchMeetingAsyncTest()
        {
            TestCommon.Instance.Mocking = false;
            using (var context = TestCommon.Instance.GetContext(TestCommon.TestSite))
            {
                var me = await context.Me.GetBatchAsync(o => o.Chats);
                await context.ExecuteAsync();
                Assert.IsTrue(me.Result.Chats.Length > 0);

                var chatQuery = me.Result.Chats.FirstOrDefault(i => i.ChatType == ChatTypeConstants.Meeting);
                Assert.IsNotNull(chatQuery);

                var chat = await chatQuery.GetBatchAsync(o => o.Messages);
                await context.ExecuteAsync();
                var chatMessages = chat.Result.Messages;

                Assert.IsNotNull(chatMessages);

                // assume as if there are no chat messages
                // There appears to be no remove option yet in this feature - so add a recognisable message
                var body = $"Hello, this is a unit test (AddChatMessageBatchMeetingAsyncTest) posting a message - PnP Rocks! - Woah...";
                await chatMessages.AddBatchAsync(body, ChatMessageContentType.Text, "Batch Test");
                await context.ExecuteAsync();

                chat = chatQuery.GetBatch(o => o.Messages);
                await context.ExecuteAsync();
                var updateMessages = chat.Result.Messages;

                var message = updateMessages.AsRequested().FirstOrDefault(o => o.Body.Content == body);

                Assert.IsFalse(message == default);
                Assert.IsNotNull(message.CreatedDateTime);
                // Depending on regional settings this check might fail
                //Assert.AreEqual(message.DeletedDateTime, DateTime.MinValue);
                Assert.IsNotNull(message.Etag);
                Assert.IsNotNull(message.Importance);
                Assert.IsNotNull(message.LastModifiedDateTime);
                Assert.IsNotNull(message.Locale);
                Assert.IsNotNull(message.MessageType);

                Assert.IsTrue(message.IsPropertyAvailable(o => o.ReplyToId));
                Assert.IsNull(message.ReplyToId);
                Assert.IsTrue(message.IsPropertyAvailable(o => o.Subject));
                Assert.IsNotNull(message.Subject);
                Assert.IsTrue(message.IsPropertyAvailable(o => o.Summary));
                Assert.IsNull(message.Summary);

            }
        }

        [TestMethod]
        public void AddChatMessageBatchOptionsMeetingTest()
        {
            //TestCommon.Instance.Mocking = false;
            using (var context = TestCommon.Instance.GetContext(TestCommon.TestSite))
            {
                var me = context.Me.GetBatch(o => o.Chats);
                context.Execute();
                Assert.IsTrue(me.Result.Chats.Length > 0);

                var chatQuery = me.Result.Chats.FirstOrDefault(i => i.ChatType == ChatTypeConstants.Meeting);
                Assert.IsNotNull(chatQuery);

                var chat = chatQuery.GetBatch(o => o.Messages);
                context.Execute();
                var chatMessages = chat.Result.Messages;

                Assert.IsNotNull(chatMessages);

                // assume as if there are no chat messages
                // There appears to be no remove option yet in this feature - so add a recognisable message
                var body = $"Hello, this is a unit test (AddChatMessageBatchOptionsMeetingTest) posting a message - PnP Rocks! - Woah...";

                chatMessages.AddBatch(new ChatMessageOptions()
                {
                    Content = body,
                    Subject = "ChatMessageBatchOptionsTest"
                });
                context.Execute();


                chat = chatQuery.GetBatch(o => o.Messages);
                context.Execute();
                var updateMessages = chat.Result.Messages;

                var message = updateMessages.AsRequested().FirstOrDefault(o => o.Body.Content == body);

                Assert.IsFalse(message == default);
                Assert.IsNotNull(message.CreatedDateTime);
                // Depending on regional settings this check might fail
                //Assert.AreEqual(message.DeletedDateTime, DateTime.MinValue);
                Assert.IsNotNull(message.Etag);
                Assert.IsNotNull(message.Importance);
                Assert.IsNotNull(message.LastModifiedDateTime);
                Assert.IsNotNull(message.Locale);
                Assert.IsNotNull(message.MessageType);

                Assert.IsTrue(message.IsPropertyAvailable(o => o.ReplyToId));
                Assert.IsNull(message.ReplyToId);
                Assert.IsTrue(message.IsPropertyAvailable(o => o.Subject));
                Assert.IsNotNull(message.Subject);
                Assert.IsTrue(message.IsPropertyAvailable(o => o.Summary));
                Assert.IsNull(message.Summary);

            }
        }

        [TestMethod]
        public async Task AddChatMessageBatchOptionsMeetingAsyncTest()
        {
            //TestCommon.Instance.Mocking = false;
            using (var context = TestCommon.Instance.GetContext(TestCommon.TestSite))
            {
                var me = await context.Me.GetBatchAsync(o => o.Chats);
                await context.ExecuteAsync();
                Assert.IsTrue(me.Result.Chats.Length > 0);

                var chatQuery = me.Result.Chats.FirstOrDefault(i => i.ChatType == ChatTypeConstants.Meeting);
                Assert.IsNotNull(chatQuery);

                var chat = await chatQuery.GetBatchAsync(o => o.Messages);
                await context.ExecuteAsync();
                var chatMessages = chat.Result.Messages;

                Assert.IsNotNull(chatMessages);

                // assume as if there are no chat messages
                // There appears to be no remove option yet in this feature - so add a recognisable message
                var body = $"Hello, this is a unit test (AddChatMessageBatchOptionsMeetingAsyncTest) posting a message - PnP Rocks! - Woah...";
                await chatMessages.AddBatchAsync(new ChatMessageOptions()
                {
                    Content = body,
                    Subject = "AddChatMessageBatchOptionsAsyncTest"
                });
                await context.ExecuteAsync();

                chat = chatQuery.GetBatch(o => o.Messages);
                await context.ExecuteAsync();
                var updateMessages = chat.Result.Messages;

                var message = updateMessages.AsRequested().FirstOrDefault(o => o.Body.Content == body);

                Assert.IsFalse(message == default);
                Assert.IsNotNull(message.CreatedDateTime);
                // Depending on regional settings this check might fail
                //Assert.AreEqual(message.DeletedDateTime, DateTime.MinValue);
                Assert.IsNotNull(message.Etag);
                Assert.IsNotNull(message.Importance);
                Assert.IsNotNull(message.LastModifiedDateTime);
                Assert.IsNotNull(message.Locale);
                Assert.IsNotNull(message.MessageType);

                Assert.IsTrue(message.IsPropertyAvailable(o => o.ReplyToId));
                Assert.IsNull(message.ReplyToId);
                Assert.IsTrue(message.IsPropertyAvailable(o => o.Subject));
                Assert.IsNotNull(message.Subject);
                Assert.IsTrue(message.IsPropertyAvailable(o => o.Summary));
                Assert.IsNull(message.Summary);

            }
        }

        [TestMethod]
        public void AddChatMessageSpecificMeetingBatchTest()
        {
            //TestCommon.Instance.Mocking = false;
            using (var context = TestCommon.Instance.GetContext(TestCommon.TestSite))
            {
                var batch = context.NewBatch();
                var me = context.Me.GetBatch(batch, o => o.Chats);
                context.Execute(batch);
                Assert.IsTrue(me.Result.Chats.Length > 0);

                var chatQuery = me.Result.Chats.FirstOrDefault(i => i.ChatType == ChatTypeConstants.Meeting);
                Assert.IsNotNull(chatQuery);

                var chat = chatQuery.GetBatch(batch, o => o.Messages);
                context.Execute(batch);
                var chatMessages = chat.Result.Messages;

                Assert.IsNotNull(chatMessages);

                // assume as if there are no chat messages
                // There appears to be no remove option yet in this feature - so add a recognisable message
                var body = $"Hello, this is a unit test (AddChatMessageSpecificMeetingBatchTest) posting a message - PnP Rocks! - Woah...";
                chatMessages.AddBatch(batch, body);
                context.Execute(batch);

                var batch2 = context.NewBatch();
                chat = chatQuery.GetBatch(batch2, o => o.Messages);
                context.Execute(batch2);
                var updateMessages = chat.Result.Messages.AsRequested();

                var message = updateMessages.First(o => o.Body.Content == body);
                Assert.IsNotNull(message.CreatedDateTime);

                // Depending on regional settings this check might fail
                //Assert.AreEqual(message.DeletedDateTime, DateTime.MinValue);
                Assert.IsNotNull(message.Etag);
                Assert.IsNotNull(message.Importance);
                Assert.IsNotNull(message.LastModifiedDateTime);
                Assert.IsNotNull(message.Locale);
                Assert.IsNotNull(message.MessageType);

                Assert.IsTrue(message.IsPropertyAvailable(o => o.ReplyToId));
                Assert.IsNull(message.ReplyToId);
                Assert.IsTrue(message.IsPropertyAvailable(o => o.Subject));
                Assert.IsNull(message.Subject);
                Assert.IsTrue(message.IsPropertyAvailable(o => o.Summary));
                Assert.IsNull(message.Summary);
            }
        }

        [TestMethod]
        public async Task AddChatMessageSpecificBatchMeetingAsyncTest()
        {
            //TestCommon.Instance.Mocking = false;
            using (var context = TestCommon.Instance.GetContext(TestCommon.TestSite))
            {
                var batch = context.NewBatch();
                var me = await context.Me.GetBatchAsync(batch, o => o.Chats);
                await context.ExecuteAsync(batch);
                Assert.IsTrue(me.Result.Chats.Length > 0);

                var chatQuery = me.Result.Chats.FirstOrDefault(i => i.ChatType == ChatTypeConstants.Meeting);
                Assert.IsNotNull(chatQuery);

                var chat = await chatQuery.GetBatchAsync(batch, o => o.Messages);
                await context.ExecuteAsync(batch);
                var chatMessages = chat.Result.Messages;

                Assert.IsNotNull(chatMessages);

                // assume as if there are no chat messages
                // There appears to be no remove option yet in this feature - so add a recognisable message
                var body = $"Hello, this is a unit test (AddChatMessageSpecificBatchMeetingAsyncTest) posting a message - PnP Rocks! - Woah...";
                await chatMessages.AddBatchAsync(batch, body, ChatMessageContentType.Text, "Batch Async Test");
                await context.ExecuteAsync(batch);

                var batch2 = context.NewBatch();
                chat = await chatQuery.GetBatchAsync(batch2, o => o.Messages);
                await context.ExecuteAsync(batch2);
                var updateMessages = chat.Result.Messages.AsRequested();

                var message = updateMessages.First(o => o.Body.Content == body);
                Assert.IsNotNull(message.CreatedDateTime);

                // Depending on regional settings this check might fail
                //Assert.AreEqual(message.DeletedDateTime, DateTime.MinValue);
                Assert.IsNotNull(message.Etag);
                Assert.IsNotNull(message.Importance);
                Assert.IsNotNull(message.LastModifiedDateTime);
                Assert.IsNotNull(message.Locale);
                Assert.IsNotNull(message.MessageType);

                Assert.IsTrue(message.IsPropertyAvailable(o => o.ReplyToId));
                Assert.IsNull(message.ReplyToId);
                Assert.IsTrue(message.IsPropertyAvailable(o => o.Subject));
                Assert.IsNotNull(message.Subject);
                Assert.IsTrue(message.IsPropertyAvailable(o => o.Summary));
                Assert.IsNull(message.Summary);
            }
        }

        [TestMethod]
        [ExpectedException(typeof(ArgumentNullException))]
        public void AddChatMessageBatchOptionsMeetingExceptionTest()
        {
            //TestCommon.Instance.Mocking = false;
            using (var context = TestCommon.Instance.GetContext(TestCommon.TestSite))
            {
                var batch = context.NewBatch();
                var me = context.Me.GetBatch(batch, o => o.Chats);
                context.Execute(batch);
                Assert.IsTrue(me.Result.Chats.Length > 0);

                var chatQuery = me.Result.Chats.FirstOrDefault(i => i.ChatType == ChatTypeConstants.Meeting);
                Assert.IsNotNull(chatQuery);

                var chat = chatQuery.GetBatch(batch, o => o.Messages);
                context.Execute(batch);
                var chatMessages = chat.Result.Messages;

                Assert.IsNotNull(chatMessages);

                // assume as if there are no chat messages
                // There appears to be no remove option yet in this feature - so add a recognisable message
                var body = string.Empty;
                chatMessages.AddBatch(batch, options: null);
                context.Execute(batch);

            }
        }

        [TestMethod]
        [ExpectedException(typeof(ArgumentNullException))]
        public void AddChatMessageBatchMeetingExceptionTest()
        {
            //TestCommon.Instance.Mocking = false;
            using (var context = TestCommon.Instance.GetContext(TestCommon.TestSite))
            {
                var batch = context.NewBatch();
                var me = context.Me.GetBatch(batch, o => o.Chats);
                context.Execute(batch);
                Assert.IsTrue(me.Result.Chats.Length > 0);

                var chatQuery = me.Result.Chats.FirstOrDefault(i => i.ChatType == ChatTypeConstants.Meeting);
                Assert.IsNotNull(chatQuery);

                var chat = chatQuery.GetBatch(batch, o => o.Messages);
                context.Execute(batch);
                var chatMessages = chat.Result.Messages;

                Assert.IsNotNull(chatMessages);

                // assume as if there are no chat messages
                // There appears to be no remove option yet in this feature - so add a recognisable message
                var body = string.Empty;
                chatMessages.AddBatch(batch, body);
                context.Execute(batch);

            }
        }

        [TestMethod]
        [ExpectedException(typeof(ArgumentNullException))]
        public void AddChatMessageMeetingExceptionTest()
        {
            //TestCommon.Instance.Mocking = false;
            using (var context = TestCommon.Instance.GetContext(TestCommon.TestSite))
            {
                var me = context.Me.Get(o => o.Chats);
                Assert.IsTrue(me.Chats.Length > 0);

                var chat = me.Chats.AsRequested().FirstOrDefault(i => i.ChatType == ChatTypeConstants.Meeting);
                Assert.IsNotNull(chat);

                chat.Load(o => o.Messages);
                var chatMessages = chat.Messages;

                Assert.IsNotNull(chatMessages);

                // assume as if there are no chat messages
                // There appears to be no remove option yet in this feature - so add a recognisable message
                var body = string.Empty;
                chatMessages.Add(body);
            }
        }
    }
}
