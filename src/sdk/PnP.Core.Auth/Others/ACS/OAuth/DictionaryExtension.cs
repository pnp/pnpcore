...
namespace PnP.Core.Auth.ACS.OAuth
{
    public static class DictionaryExtension
    {
        ...
        {
            var dictionary = JsonSerializer.Deserialize<System.Collections.Generic.Dictionary<string, object>>(encodedDictionary);
            ...
            foreach (System.Collections.Generic.KeyValuePair<string, object> current in dictionary)
            {
                ...
                {
                    self.Add(current.Key, JsonSerializer.Serialize(current.Value));
                }
                else
                {
