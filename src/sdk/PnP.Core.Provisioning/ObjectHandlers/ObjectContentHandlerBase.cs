...
namespace PnP.Core.Provisioning.ObjectHandlers
{
    ...
    internal abstract class ObjectContentHandlerBase : ObjectHandlerBase
    {
        ...
        internal async Task<Model.File> RetrieveFieldValuesAsync(PnPContext context, IFile file, Model.File modelFile)
        {
            ...
            {
                // Not every file has a list item (a file in a non-library folder, for instance).
                return modelFile;
            }
            ...
            foreach (KeyValuePair<string, object> fieldValue in listItem.Values.Where(f => !FieldsToExclude.Contains(f.Key)))
            {
                ...
                switch (field.TypeAsString)
                {
                    case "URL":
                        ...
                        break;
                    case "User":
                        // IFieldUserValue surfaces the address as Email; CSOM called the same
                        // thing FieldUserValue.Email too, so this is a rename only in my first draft.
                        value = (fieldValue.Value as IFieldUserValue)?.Email;
                        break;
                    case "LookupMulti":
                    case "TaxonomyFieldType":
                    case "TaxonomyFieldTypeMulti":
                        // PnP Framework serialized these with its Newtonsoft based JsonUtility.
                        // System.Text.Json cannot serialize PnP Core's field value interfaces
                        // directly, so the properties the template actually needs are projected
                        // explicitly - see SerializeFieldValue.
        ...
        private static string SerializeFieldValue(object fieldValue)
        {
            switch (fieldValue)
            {
                ...
                case IFieldTaxonomyValue taxonomy:
                    // IFieldTaxonomyValue exposes only TermId and Label. CSOM's
                    // TaxonomyFieldValue also carried WssId - the taxonomy hidden list row id -
                    // which is per-site bookkeeping and meaningless in a portable template, so
