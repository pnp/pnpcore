...
namespace PnP.Core.Provisioning.ObjectHandlers
{
    ...
    internal partial class ObjectListInstance : ObjectHandlerBase
    {
            ...
            TokenParser parser, ApplyConfiguration configuration)
        {
            ...
            // Read once for the whole template rather than once per list - it is the same web either
            // way, and a site's available content types run to several hundred.
            ...
            foreach (ListInstance templateList in template.Lists)
            {
                ...
                // Per list, so one list cannot abandon the rest of the template. Every other handler
                // reports and carries on - this one did not, and a single unsupported setting on the
                // first of five lists took the whole apply down along with the fourteen handlers
            ...
            ProvisioningTemplate template, ListInstance templateList, HashSet<string> availableContentTypes)
        {
            ...
            foreach (ContentTypeBinding binding in templateList.ContentTypeBindings)
            {
                ...
                if (!found)
                {
                    // The counts are part of the message on purpose. "Not available" and "we only
                    // looked at the first page of what is available" are the same sentence otherwise,
                    // and telling them apart afterwards costs a whole run against a live tenant.
                    throw new Exception(
