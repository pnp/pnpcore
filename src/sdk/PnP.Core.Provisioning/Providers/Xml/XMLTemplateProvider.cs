...
namespace PnP.Core.Provisioning.Providers.Xml
{
    ...
    public abstract class XMLTemplateProvider : TemplateProviderBase
    {
        ...
        public override List<ProvisioningTemplate> GetTemplates(ITemplateFormatter formatter)
        {
            ...
            foreach (var file in files)
            {
                if (file.EndsWith(".xml", StringComparison.InvariantCultureIgnoreCase))
                {
                    ...
                    {
                        // MIGRATION PHASE 3 (ILogger + PnPCoreProvisioningResources): Log.Warning(Constants.LOGGING_SOURCE_FRAMEWORK_PROVISIONING, CoreResources.Provisioning_Providers_XML_InvalidFileFormat, file);
                        continue;
                    }
        ...
        public override ProvisioningTemplate GetTemplate(string uri, string identifier, ITemplateFormatter formatter, ITemplateProviderExtension[] extensions = null)
        {
            ...
            if (stream == null)
            {
                throw new ApplicationException(string.Format(PnPCoreProvisioningResources.Provisioning_Formatter_Invalid_Template_URI, uri));
            }
