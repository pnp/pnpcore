        ...
        private T GetXamlSerializedPackagePartValue<T>(PackagePart part) where T : class
        {
            ...
            using (Stream stream = part.GetStream(FileMode.Open))
            {
                ...
                using (var streamReader = new StreamReader(stream))
                {
                    ...
                    {
                        // Backward compatibility shim for .pnp packages produced by the original
                        // OfficeDevPnP.Core: the XAML manifest names the type by clr-namespace and
                        // assembly, so it is rewritten to point at this assembly before XAML loads it.
                        // Both halves must move together - PnP Framework rewrote to
                        // "PnP.Framework.Provisioning...;assembly=PnP.Framework".
                        textContent = textContent.Replace("clr-namespace:OfficeDevPnP.Core.Framework.Provisioning.Connectors.OpenXML.Model;assembly=OfficeDevPnP.Core", "clr-namespace:PnP.Core.Provisioning.Connectors.OpenXML.Model;assembly=PnP.Core.Provisioning");
