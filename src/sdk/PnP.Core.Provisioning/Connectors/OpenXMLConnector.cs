...
using PnP.Core.Provisioning.Connectors.OpenXML.Model;
using System;
    ...
    public class OpenXMLConnector : FileConnectorBase, ICommitableFileConnector
    {
        ...
        public override void SaveFileStream(string fileName, string container, Stream stream)
        {
            ...
            try
            {
                ...
                // MIGRATION PHASE 3 (ILogger + PnPCoreProvisioningResources): Log.Info(Constants.LOGGING_SOURCE, CoreResources.Provisioning_Connectors_OpenXML_FileSaved, fileName, container);
            }
            catch
            {
                // MIGRATION PHASE 3: restore as
                //   catch (Exception ex) { Log.Error(Constants.LOGGING_SOURCE, CoreResources.Provisioning_Connectors_OpenXML_FileSaveFailed, fileName, container, ex.Message); throw; }
                // Declared bare for now so the exception variable does not sit unused; the rethrow
        ...
        public override void DeleteFile(string fileName, string container)
        {
            ...
            try
            {
                ...
                if (file != null)
                {
                    ...
                    // MIGRATION PHASE 3 (ILogger + PnPCoreProvisioningResources): Log.Info(Constants.LOGGING_SOURCE, CoreResources.Provisioning_Connectors_OpenXML_FileDeleted, fileName, container);
                }
                else
                {
                    // MIGRATION PHASE 3 (ILogger + PnPCoreProvisioningResources): Log.Warning(Constants.LOGGING_SOURCE, CoreResources.Provisioning_Connectors_OpenXML_FileDeleteNotFound, fileName, container);
                }
            }
            catch
            {
                // MIGRATION PHASE 3: restore as
                //   catch (Exception ex) { Log.Error(Constants.LOGGING_SOURCE, CoreResources.Provisioning_Connectors_OpenXML_FileDeleteFailed, fileName, container, ex.Message); throw; }
                // Declared bare for now so the exception variable does not sit unused; the rethrow
                // preserves the original behaviour.
                throw;
            }
        ...
        private MemoryStream GetFileFromStorage(string fileName, string container)
        {
            try
            {
                ...
                if (file != null)
                {
                    // MIGRATION PHASE 3 (ILogger + PnPCoreProvisioningResources): Log.Info(Constants.LOGGING_SOURCE, CoreResources.Provisioning_Connectors_OpenXML_FileRetrieved, fileName, container);
            ...
            catch (Exception ex)
            {
                ...
                {
                    // MIGRATION PHASE 3 (ILogger + PnPCoreProvisioningResources): Log.Error(Constants.LOGGING_SOURCE, CoreResources.Provisioning_Connectors_OpenXML_FileNotFound, fileName, container, ex.Message);
                    return null;
                }
