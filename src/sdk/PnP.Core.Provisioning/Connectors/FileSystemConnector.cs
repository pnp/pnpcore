    ...
    public class FileSystemConnector : FileConnectorBase
    {
        ...
        public override void SaveFileStream(string fileName, string container, Stream stream)
        {
            ...
            try
            {
                ...
                // MIGRATION PHASE 3 (ILogger + PnPCoreProvisioningResources): Log.Info(Constants.LOGGING_SOURCE, CoreResources.Provisioning_Connectors_FileSystem_FileSaved, fileName, container);
            }
            catch
            {
                // MIGRATION PHASE 3: restore as
                //   catch (Exception ex) { Log.Error(Constants.LOGGING_SOURCE, CoreResources.Provisioning_Connectors_FileSystem_FileSaveFailed, fileName, container, ex.Message); throw; }
                // once ILogger and PnPCoreProvisioningResources exist. Declared bare for now so the
                // exception variable does not sit unused; the rethrow preserves the original behaviour.
                throw;
            }
        }
        ...
        public override void DeleteFile(string fileName, string container)
        {
            ...
            try
            {
                ...
                if (File.Exists(filePath))
                {
                    ...
                    // MIGRATION PHASE 3 (ILogger + PnPCoreProvisioningResources): Log.Info(Constants.LOGGING_SOURCE, CoreResources.Provisioning_Connectors_FileSystem_FileDeleted, fileName, container);
                }
                else
                {
                    // MIGRATION PHASE 3 (ILogger + PnPCoreProvisioningResources): Log.Warning(Constants.LOGGING_SOURCE, CoreResources.Provisioning_Connectors_FileSystem_FileDeleteNotFound, fileName, container);
                }
            }
            catch
            {
                // MIGRATION PHASE 3: restore as
                //   catch (Exception ex) { Log.Error(Constants.LOGGING_SOURCE, CoreResources.Provisioning_Connectors_FileSystem_FileDeleteFailed, fileName, container, ex.Message); throw; }
                // once ILogger and PnPCoreProvisioningResources exist. Declared bare for now so the
                // exception variable does not sit unused; the rethrow preserves the original behaviour.
                throw;
            }
        ...
        private MemoryStream GetFileFromStorage(string fileName, string container)
        {
            try
            {
                ...
                // MIGRATION PHASE 3 (ILogger + PnPCoreProvisioningResources): Log.Info(Constants.LOGGING_SOURCE, CoreResources.Provisioning_Connectors_FileSystem_FileRetrieved, fileName, container);
                stream.Position = 0;
                return stream;
            }
            catch (Exception ex)
            {
                ...
                {
                    // MIGRATION PHASE 3 (ILogger + PnPCoreProvisioningResources): Log.Error(Constants.LOGGING_SOURCE, CoreResources.Provisioning_Connectors_FileSystem_FileNotFound, fileName, container, ex.Message);
                    return null;
                }
