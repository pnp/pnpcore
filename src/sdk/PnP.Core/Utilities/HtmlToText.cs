...
namespace PnP.Core.Utilities
{
    ...
    internal static class HtmlToText
    {
        ...
        internal static string ConvertSimpleHtmlToText(string html, int maxLength)
        {

            return HtmlDecodeCore(html, maxLength, null);
        }

        private static string HtmlDecodeCore(string html, int maxLength, IList<string> tagsToRetain)
        {
        ...
        private static string ProceedToEndOfHtmlString(string html, ref int currentPosition)
        {
            char ch = html[currentPosition];
        ...
        private static string ProceedToEndOfTag(string html, IList<string> tagsToRetain, ref int currentPosition)
        {
            char ch = html[currentPosition];
            int num = currentPosition;
            while ((ch != '>') && (num < (html.Length - 1)))
