...
- Extract the PnP Core SDK into YML based API documentation: use `dotnet docfx metadata docfx.json`
- Build the documentation: in this step the YML API documentation and the articles are merged into a documentation site. Use `dotnet docfx build docfx.json` and then check the generated `_site` folder
- To check locally, Use `dotnet docfx build docfx.json --serve` to build the site and host on http://localhost:8080
...
> [!Note]
> Use `dotnet tool update -g docfx` to install the latest [DocFX](https://dotnet.github.io/docfx/)`
